...
<p align="center">
  <a href="https://github.com/Treeweft/treeweft"><img src="https://img.shields.io/badge/github-treeweft-3fb950?logo=github" alt="GitHub"></a>
  <a href="https://github.com/Treeweft/treeweft/actions/workflows/ci.yml"><img src="https://github.com/Treeweft/treeweft/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
...
three current agent models, an agent using treeweft matched the answer quality of a
...
little vocabulary with the actual code — Treeweft asks the LLM to *hallucinate*
...
> subprocess spawned by your MCP client (`treeweft-mcp`). The deprecated
> HTTP+SSE transport is available with `COMPOSE_PROFILES=http-mcp`.

Prebuilt images for the Treeweft services (`treeweft/indexer`,
`treeweft/mcp-server`, `treeweft/ui`, `treeweft/qwen3-reranker`) are published
...
Point your MCP client at the `treeweft-mcp` console script — it reaches the
...
`-H "Authorization: Bearer $TREEWEFT_KEY"`; search is also checked against
per-repo grants. `/health`, `/status`, and `GET /jobs` stay open (job views are
redacted for anonymous callers). Set `TREEWEFT_SEARCH_OPEN=1` to keep `/search`
...
**Short answer.** Across three current agent models, an agent using treeweft matched
...
on symbol-free questions for every model. Treeweft's answer-quality score was higher in
...
| agent model | questions | cost vs grep (per-query p) | turns (grep → treeweft) | 12-turn caps (grep / treeweft) | correctness (grep → treeweft, p) |
|---|---|---|---|---|---|
...
- **Treeweft is cheaper.** The per-query cost difference is significant in 5 of 6 rows
...
- **On symbol-free questions grep runs out of road.** It hit the 12-turn cap on 11–24%
  of questions, significantly more often than treeweft on every model. Capable agents
...
- **On symbol-bearing questions grep is hard to beat at finding the file** — it
  ...
  Treeweft still gets there for less.
- **vs vector RAG with identical embeddings** (zilliztech/claude-context,
  deepseek-flash): treeweft ranks the right file first significantly more often —
...
the question, grep finds it directly and treeweft's advantage is cost alone (11–20%
cheaper). When you can only describe the behaviour, grep has nothing to match and
flounders; treeweft's cost advantage widens to 25–47%. *Don't use a semantic engine for
symbol lookup — grep already nails it.* Use treeweft for "how does this work?" and
...
**three.js** (JS), n=100 per repo, agent `deepseek-chat` — found treeweft **significantly
...
the models above, and judging its own answers) found treeweft significantly better on
...
- A treeweft search takes roughly 3 seconds, most of it reranking — latency traded for
  fewer agent turns.
- Raw per-query result rows are not shipped in the repo; the query sets and gold answers
  are. Reproduce with `python -m treeweft.benchmark agentic --help`.
...
Treeweft emits OpenTelemetry traces from the indexer for every job and
...
`assets/grafana/treeweft-indexer-traces.json` for the pre-built per-stage
dashboard. Configure via the standard
`OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_TRACES_SAMPLER` env vars;
span names use the stable `treeweft.` attribute prefix. Set
...
benchmark harness (`python -m treeweft.benchmark run|ab|agentic`).
...
evaluating, operating, and changing Treeweft.

## Brand

See [assets/branding/](assets/branding/) for logo files, favicon, and full brand
guidelines. The Thread-Tree mark is the canonical Treeweft symbol.
