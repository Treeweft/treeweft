...
# MCP access is stdio by default (treeweft-mcp, no container); the
...
if [ "${TREEWEFT_PROFILE:-}" != "simple" ]; then
    echo "WARNING: TREEWEFT_PROFILE is not 'simple' in .env — this script" >&2
...
if [[ "${INDEXER_HOST}" == 127.* || "${INDEXER_HOST}" == "localhost" ]]; then
    ...
    echo "AUTH_ENABLED=true (plus TREEWEFT_HMAC_SECRET) in .env."
fi
echo "====================="
echo ""
uvicorn treeweft.indexer_service:app --host "${INDEXER_HOST}" --port "${INDEXER_PORT}"
