/** Local-dev fallback when `window.__TREEWEFT_API_BASE__` is unset. */
...
/**
 * Resolve the API base. Reads `window.__TREEWEFT_API_BASE__` (set by
...
export function resolveApiBase(
  ...
  const base = win?.__TREEWEFT_API_BASE__;
...
async function request<T>(
  ...
  // Cookie/session auth: send the `treeweft_session` cookie cross-origin.
