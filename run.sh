...
# TREEWEFT_CORS_ORIGINS to the UI origin (explicit origin + allow-credentials).
...
if [ -z "${TREEWEFT_CORS_ORIGINS:-}" ]; then
    export TREEWEFT_CORS_ORIGINS="http://localhost:${UI_PORT}"
fi
...
#                `treeweft-mcp` console script, which this script does not
