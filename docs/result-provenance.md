...
Every search hit in Treeweft carries enough information to trace it back to its
...
commit it came from is still the tip of the branch*. Treeweft captures the
...
The `ChunkHit` contract is defined in `src/treeweft/domain/shared.py`.
