...
(`c16207823d7b26e25bcbbe2a2eb032b602eea519`). **Harness:** treeweft v0.3.0 (this repository).
...
(grep/glob/read_file tools), `treeweft` (treeweft's `search_code`) or `claude-context`
...
runbook (A = all three arms on the cheapest model, B = grep vs treeweft on each frontier
...
Across three models and two query populations, **treeweft answered as well as a
...
model (p ≤ 5.6e-07); on symbol-free queries treeweft also exhausts the turn cap
...
## Headline: treeweft vs grep

Tokens and cost are treeweft's change relative to grep (negative = treeweft cheaper).
W/L/T are per-query wins/losses/ties for treeweft; p is a two-sided sign test with ties excluded.

| Model | Query set | Tokens | Cost | Turns (grep → treeweft) | Turn-cap hits (grep / treeweft) | Correctness W/L/T | Correctness p | Recall@5 W/L | Recall@5 p |
|---|---|---|---|---|---|---|---|---|---|
...
(treeweft wins/losses vs grep, ties excluded):

**Cost per query** (win = treeweft cheaper on that query):
...
**Turn-cap exhaustion on symbol-free queries** (win = grep capped and treeweft did not):
...
**Ranking, treeweft vs grep.** Recall@1 and MRR are statistical ties in five cells. In the
...
**Ranking, treeweft vs claude-context** (deepseek-flash, same embedding model): treeweft
...
| treeweft | 4.49 | 0.68 | 0.88 | 0.773 | 3.6 | 15,094 | $0.0028 | 0 |
...
| treeweft | 4.56 | 0.44 | 0.70 | 0.575 | 4.2 | 28,623 | $0.0044 | 4 |
...
| treeweft | 4.47 | 0.69 | 0.88 | 0.785 | 3.3 | 18,527 | $0.0340 | 1 |
...
| treeweft | 4.43 | 0.41 | 0.74 | 0.563 | 3.3 | 23,795 | $0.0368 | 3 |
...
| treeweft | 4.38 | 0.74 | 0.93 | 0.828 | 5.0 | 21,080 | $0.0724 | 6 |
...
| treeweft | 4.41 | 0.50 | 0.76 | 0.628 | 5.2 | 20,805 | $0.0721 | 8 |
...
3. **Grep's failure mode on symbol-free queries is exhaustion, not wrong answers.**
   It hit the 12-turn cap on 11, 11 and 24 of 100 queries (vs treeweft's 4, 3, 8), yet
...
| treeweft index | 6,424 chunks, 1,943 files, 0 indexing errors |
...
| Harness defaults | `--max-turns 12`, `--obs-char-limit 4000`, scoped treeweft search — matching the pinned anchor cells |
...
python -m treeweft.benchmark agentic \
  ...
  --arms grep,treeweft \                                          # Cell A adds claude-context
