...
This document describes how Treeweft decides who may read which repository's
...
The pure functions live in `src/treeweft/domain/authorization/access.py` and are
...
1. MCP client → MCP server tool. There are **two distinct credential
   ...
   - **Write/admin tools** (`index_*`, `remove_indexed_source`,
     ...
     via `_caller_auth_headers`) → `TREEWEFT_MCP_TOKEN` (the caller's own
     credential supplied via the environment — the stdio credential model)
     → the gated service account (`TREEWEFT_MCP_API_KEY`, only when
     `TREEWEFT_MCP_SERVICE_ACCOUNT=true`) → nothing. Off by default; on a
   ...
   - **Read tools** — the nine per-repo-scoped ones: `search_code`,
     ...
     `TREEWEFT_MCP_SERVICE_ACCOUNT=true` for write convenience would
     ...
     *cross-repo* visibility instead of their own scoped grants — reopening,
     ...
     exists to prevent. `TREEWEFT_MCP_TOKEN` is safe on the read path
...
2. Indexer read endpoint (`src/treeweft/application/indexer_service.py`) calls
   `_require_scope_xor` then **`_authorize_scope(request, source_id, action)`**
   (`src/treeweft/application/indexer_authz.py`) — the single choke point.
   - If `AUTH_ENABLED != true`, or `TREEWEFT_SEARCH_OPEN=1` is set: returns
   ...
   is what makes `TREEWEFT_SEARCH_OPEN` possible). `/graph-explore` and the
...
| `TREEWEFT_SEARCH_OPEN` | With auth on, keep **search** anonymous (protect indexing only). |
...
session, sets an **HttpOnly, SameSite=Lax** cookie named `treeweft_session`
...
1. Resolve `client_ip` from `request.client.host`. `X-Forwarded-For` is
   trusted **only** when `TREEWEFT_LOGIN_TRUST_FORWARDED_FOR=1` (set this only
   behind a trusted reverse proxy; otherwise a client could spoof the header
   to dodge the per-IP limit).
2. Count failed attempts in the last `TREEWEFT_LOGIN_WINDOW_SECONDS`. If that
   count is `>= TREEWEFT_LOGIN_MAX_ATTEMPTS`, return **429** with a
...
| --- | --- | --- |
| `TREEWEFT_LOGIN_MAX_ATTEMPTS` | `5` | Failed logins per `(username, client_ip)` before 429 |
| `TREEWEFT_LOGIN_MAX_ATTEMPTS_PER_USERNAME` | `50` | Failed logins per username across **all** IPs (defeats IP rotation; keep well above the per-IP limit so users behind one NAT aren't locked out) |
| `TREEWEFT_LOGIN_WINDOW_SECONDS` | `300` | Sliding-window length in seconds |
| `TREEWEFT_LOGIN_TRUST_FORWARDED_FOR` | `0` | `1` → key the limiter on `X-Forwarded-For` (trusted-proxy only) |
...
1. **Session cookie** `treeweft_session` — HMAC the value, look up in
...
fallback, so existing deployments that set `TREEWEFT_ADMIN_KEY` continue to
authenticate without any migration action.

### First-admin bootstrap

If `users` is empty at indexer startup and both `TREEWEFT_BOOTSTRAP_ADMIN_USER`
and `TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD` are set, an admin user with a bcrypt
