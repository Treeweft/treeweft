...
`run.sh` does, in order: `pip install -e .`, `docker compose build mcp-server ui`, brings up `postgres`, `tei-embedding` (or `tei-embedding-cpu` with the `cpu` profile), `tei-reranker`, and `ui` — plus `neo4j` + `milvus` when `COMPOSE_PROFILES` includes `local-infra` (the quickstart default), `qwen3-reranker` with the `qwen3` profile, and the `mcp-server` container (deprecated HTTP+SSE transport) with the `http-mcp` profile — waits for Postgres to accept TCP connections (`scripts/wait-for-postgres.sh`, bounded by `POSTGRES_WAIT_SECONDS`, default 60; a fresh volume spends several seconds in initdb and the indexer fails loud without it) — then runs `uvicorn treeweft.indexer_service:app` on port 8001 in the foreground. The indexer must run on the host (not in Docker) so it can reach arbitrary user paths. `run.sh` still builds the `mcp-server` image (cheap, keeps the opt-in path ready) but does NOT start that container unless `COMPOSE_PROFILES` includes `http-mcp` — the default MCP transport is stdio via the `treeweft-mcp` console script, spawned directly by your MCP client with no container involved. If you do opt into `http-mcp`, that container reaches the indexer via `host.docker.internal:8001`.

Service ports:
- MCP server: stdio via the `treeweft-mcp` console script (no port). The
  ...
  `uvicorn treeweft.main:app` (SSE) regardless of any env var; it never reads
  `TREEWEFT_MCP_TRANSPORT`. `TREEWEFT_MCP_TRANSPORT` is read only by the
  `treeweft-mcp` console script (`mcp_stdio.main()`), and only `stdio` (the
  default) is meaningful there — do NOT set `TREEWEFT_MCP_TRANSPORT=sse` in a
  shared `.env`: `src/treeweft/__init__.py` loads the repo `.env` from any
  cwd an editable install runs in, so it would silently turn your
  `treeweft-mcp` script into a second, colliding SSE server instead of
  toggling the container.
- Indexer FastAPI: `http://localhost:8001` (host)
- TEI embedding: `8082`, TEI reranker: `8081`
- Neo4j Bolt: `7687`, HTTP: `7474` (default auth `neo4j/treeweft_pass`)
...
`TREEWEFT_PROFILE=simple` (see `docs/simple-mode.md`, `.env.simple.example`,
`./run-simple.sh`, `docker-compose.simple.yml`) is the no-GPU evaluator path:
embedded LanceDB (`~/.treeweft/lancedb`) + embedded SQLite graph
(`~/.treeweft/graph.db`) + OpenAI API for embeddings/HyDE/summaries + an
...
`src/treeweft/__init__.py`); the real switches are four selector vars usable
independently:
...
The root shims (`treeweft/retriever.py`, `graph_store.py`, `embedder.py`,
...
# TREEWEFT_SEARCH_OPEN=1) require `-H "Authorization: Bearer $TREEWEFT_KEY"`.
...
curl -X POST http://localhost:8001/index-repo \
  ...
  -H "Authorization: Bearer $TREEWEFT_KEY" \
...
curl -H "Authorization: Bearer $TREEWEFT_KEY" \
...
# Hits the indexer directly; auth via $TREEWEFT_KEY (or $TREEWEFT_MCP_API_KEY).
# Rebuilds all sources by default — do NOT trust graph_indexed here (see below).
python -m treeweft.rebuild_graphs --wait
python -m treeweft.rebuild_graphs --dry-run          # preview, enqueue nothing
python -m treeweft.rebuild_graphs --only-missing     # only graph_indexed=false (two-pass)
python -m treeweft.rebuild_graphs --source-id <id> --source-id <id2>
...
python -m treeweft.fleet onboard repos.yaml --wait        # --dry-run / --force
python -m treeweft.fleet health --staleness --entities    # or --json
curl http://localhost:8001/fleet?summary_only=true

# Indexer status (live progress while running) — public
curl http://localhost:8001/status

# The benchmark CLI is `python -m treeweft.benchmark <subcommand>` with
...
#
# For the documented grep-vs-treeweft eval workflow (clean → agentic → rollup, with
# significance), see docs/benchmark-eval.md.
#
# VERDICT (n=100 × 4 repos incl. C#/Java/JS + featbit control,,
# docs/benchmark-findings.md): treeweft is SEMANTIC search, NOT symbol lookup. On
# symbol-BEARING queries grep wins/ties (it string-matches the identifier) — don't
# benchmark only those, they understate treeweft. On symbol-FREE queries treeweft
...
# deepseek-flash/claude-sonnet-5/gpt-5.5, independent gpt-4o judge): treeweft is
...
python -m treeweft.benchmark queries --source-dir ~/source/featbit --count 20

# Retrieval-quality run against the live indexer /search (precision/recall/MRR
# to stdout). Feature-flag toggles, not modes.
python -m treeweft.benchmark run --queries benchmarks/queries/featbit-clean-100.jsonl \
...
python -m treeweft.benchmark ab --queries benchmarks/queries/featbit-clean-100.jsonl --graph

# Agentic token+quality benchmark: a real LLM agent answers each query once per
# arm — grep/glob/read_file tools, treeweft search_code, and optionally
# zilliztech/claude-context and aider's repo-map
# (--arms grep,treeweft,claude-context,repomap) — and we compare real API token
...
python -m treeweft.benchmark gold    --queries benchmarks/queries/featbit-clean-100.jsonl --limit 5
python -m treeweft.benchmark agentic --queries benchmarks/queries/featbit-clean-100.jsonl \
  --repo ~/source/featbit --search-url http://localhost:8001 \
  --arms grep,treeweft --max-turns 12 [--model M] [--native-tools|--no-native-tools] \
...
model can't follow the tool protocol) and a repo that is BOTH indexed (treeweft arm)
...
agent and judge unless `--allow-floating-model` or `TREEWEFT_ALLOW_FLOATING_MODELS=1` —
...
drift): `2026-07-05T232746_featbit_summary.json` (treeweft corr 3.89 vs grep
3.41, p=0.006, −44% tokens) and `2026-07-05T234145_featbit-symfree_summary.json`
(treeweft corr 3.23 vs grep 2.13, p=2.3e-10, −88% tokens, grep cap-rate 25.8%
...
- **Retrieval A/Bs run as an isolated second indexer** (port 8002), never against production state. Inline shell env beats `.env` (`os.environ.setdefault` in `src/treeweft/__init__.py`), so launch with overrides: `DATABASE_URL=...treeweft_ab` pointing at a fresh database — migrations auto-run, and critically the `embedding_backends` registry re-seeds from `EMBEDDING_URLS` (against the production DB that env var is IGNORED after first seeding); own `MILVUS_COLLECTION` + `VECTOR_DIM` for embedding arms; `skip_graph: true` on index jobs so the shared Neo4j (and its community embeddings) stays clean. Reranker arms need no re-index: read the production collection search-only (no `MILVUS_COLLECTION`/`VECTOR_DIM` overrides, swap `RERANKER_URL`) — but never POST `/index-*` to that instance. Drop the experiment DB and `treeweft_ab_*` Milvus collections afterward.
...
- **MCP protocol requires the full handshake** before tool calls: connect SSE → receive `session_id` → send `initialize` → receive response → send `notifications/initialized` → then `tools/call`. The SSE message endpoint (`sse.handle_post_message`) is mounted as a raw ASGI app via `Mount` in `application/main.py` — NOT a FastAPI route — because it sends its own `202` response; wrapping it in `@app.post` double-responds and breaks every call after `initialize`. The agentic benchmark's treeweft arm does NOT use MCP; it calls the indexer `POST /search` directly (`adapters/benchmark/treeweft_tool.py`).
...
- **Per-chunk summary cache** lives in Postgres (`summary_cache` table, migration `005_summary_cache.sql`, keyed by `SHA1(chunk_text) + LLM_MODEL + prompt_version`). Shared across indexer workers so horizontal scale doesn't re-pay for the same chunk summarizations. Like the JobStore, the cache helpers fail loud (`RuntimeError`) if the Postgres pool is unavailable.
- **Prompts are an append-only versioned registry** (`adapters/llm_api/prompts.py`, `operation -> {version: PromptVersion}`; ADR-003, `specs/002-prompt-versioning`). Never edit a shipped version — add a new one, with a hash entry in `tests/unit/fixtures/prompt_hashes.json` (`test_prompt_registry.py` guards this). `summary_cache` rows are written and read under the version resolved for the call, so several versions' rows coexist; a version's rows are simply never read again once every source has moved off it. Admins pin a version per operation: the deployment pin (`chunk_summary`, `hyde`), plus a per-source override for `chunk_summary` only — `hyde` is deployment-wide. Pins live in Postgres (`prompt_pins` table, migration `021_prompt_versions.sql`) and are resolved from an in-memory view refreshed via `LISTEN prompt_pins_changed` plus a `PROMPT_PINS_REFRESH_SECONDS`-second poll (default 5) as a floor, so every indexer process picks up a change within about 5 seconds — no per-chunk or per-query database round-trip. A release that registers a new version changes nothing: pins are seeded once at startup (an existing deployment to what its sources were built with, a fresh install to the latest) and move only when an admin moves them. Moving the `chunk_summary` pin — deployment-wide or a source override — enqueues a summary-only `resummarize` job per stale source instead of a re-index: it regenerates each chunk's summary and summary vector at the target version (cache hits reused) and touches nothing else (no parsing, no code embeddings, no graph work). A source is stale when its recorded `summary_prompt_version` differs from its effective version, or while an unfinished refresh's `summary_refresh_target` mark is still set. Without Postgres, resolution falls back to the baseline versions (`chunk_summary` v3, `hyde` v1) and the pin endpoints return 503. See the runbook, [`docs/prompt-versions.md`](prompt-versions.md), for trialling a version on one source, promoting it to the deployment, and troubleshooting a refresh.
