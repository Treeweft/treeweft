...
A lightweight Treeweft deployment for evaluating on limited hardware — a
...
the default; this is an additive profile selected by `TREEWEFT_PROFILE=simple`.

## What runs where

| Concern | Full mode | Simple mode |
| --- | --- | --- |
| Vector store | Milvus (server) | **LanceDB**, embedded at `~/.treeweft/lancedb` |
| Graph store | Neo4j (server) | **SQLite**, embedded at `~/.treeweft/graph.db` |
...
- **Agentic benchmark — historical, pre-2026-07-05 harness** (deepseek-chat
  ...
  has not been re-run on the current harness. Simple-mode treeweft **scores higher on answer quality** —
  ...
  full-stack treeweft arm in that same epoch measured judge 4.26 @ 19.8k
...
# Storage locations
LANCEDB_PATH=~/.treeweft/lancedb
GRAPH_DB_PATH=~/.treeweft/graph.db
...
`~/.treeweft/lancedb` and re-indexing (the store fails loud on a dim
mismatch).
...
MCP access is stdio by default (`treeweft-mcp`, spawned directly by your MCP
...
1. **Enable auth** — set all three in `.env`:
   ...
   ```bash
   AUTH_ENABLED=true
   TREEWEFT_HMAC_SECRET=<python -c "import secrets; print(secrets.token_hex(32))">
   TREEWEFT_ADMIN_KEY=<secret>
   ```

   `TREEWEFT_HMAC_SECRET` is required whenever auth is on — the indexer refuses
   ...
   `TREEWEFT_SEARCH_OPEN=1` if you want search to stay anonymous while
...
`TREEWEFT_PROFILE=simple`) and using `./run.sh`.

## How it's wired (for the curious)

`TREEWEFT_PROFILE=simple` only sets env-var defaults
(`src/treeweft/__init__.py:_apply_profile_defaults`). The actual switches are
