...
Embeddings Inference) backends. The then-current adapter (`src/treeweft/adapters/tei/embedding_adapter.py`, since
...
Introduce `EmbeddingProxy` in `src/treeweft/adapters/tei/embedding_proxy.py`:
...
- `src/treeweft/adapters/tei/embedding_proxy.py` — the proxy, the tokenizer
  estimator, and `TEIEmbeddingAdapter`.
- `src/treeweft/embedder.py` — re-export shim used by call sites.
