...
the same agent with treeweft `search_code`) on a current model generation, at
...
| tier | grep | treeweft | claude-context | repomap | 4-arm | grep+treeweft |
|---|---|---|---|---|---|---|
...
2026-06 run). It also lost decisively — judge 3.35 vs treeweft's 4.26. In that
...
**Cell A — the headline table.** `grep`, `treeweft`, `claude-context`,
...
**Cell B — "it holds across tiers".** `grep` + `treeweft` only, on each
...
worst on exactly the professional-scale codebases treeweft targets. That is a
...
**Add the symbol-free cell.** Symbol-bearing queries understate treeweft —
...
```bash
python -m treeweft.benchmark queries --source-dir <pinned corpus> \
  --llm-symbol-free --count 120 --sample-seed 0 \
  --output benchmarks/queries/<corpus>-symfree.jsonl
python -m treeweft.benchmark clean --symbol-free \
...
2. **Sufficiency screen** — one LLM call per query instead of ~6 turns
   ...
   ```python
   import asyncio
   from treeweft.application.benchmark.screen import run_screen
...
3. **Confirm prices.** Every model you plan to run must have a
   `verified: True` entry in `src/treeweft/domain/benchmark/cost_model.py`
...
**Re-index after pinning.** The `treeweft` arm reads the index; the `grep`
...
curl -X POST http://localhost:8001/index-repo \
  -H 'Content-Type: application/json' -H "Authorization: Bearer $TREEWEFT_KEY" \
...
python -m treeweft.benchmark agentic \
  ...
  --arms grep,treeweft[,claude-context,repomap] \
