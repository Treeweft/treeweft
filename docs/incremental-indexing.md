...
This guide is for operators who want a Treeweft index to stay current as code
...
unmerged PRs are acknowledged and ignored) to the Treeweft indexer's
`POST /webhook` endpoint. The handler (`handle_webhook` in
`src/treeweft/application/routes_webhook.py`):
...
3. **Checks backpressure** — if the queue depth ≥ `MAX_QUEUE_DEPTH` (env,
   ...
   increments `treeweft_webhook_shed_total`. The provider will retry.
...
compute `treeweft_merge_to_searchable_seconds`.
...
`src/treeweft/application/indexer_runners.py`) reads `jobs.payload`, reconstructs the `changed_files` list and source info, and
...
The runner observes:
- `treeweft_incremental_job_duration_seconds` — wall-clock time of the full run.
- `treeweft_incremental_jobs_total{status="done|failed|dead_letter"}` — bumped
  on completion.
- `treeweft_merge_to_searchable_seconds` — computed as `now -
...
(`src/treeweft/application/lifecycle.py`) the indexer finds `RUNNING`/`QUEUED`
...
3. If `attempts >= MAX_JOB_ATTEMPTS`: sets `jobs.status = "dead_letter"`,
   persists, and increments `treeweft_dead_letter_jobs_total`. The job is NOT
   re-enqueued.
...
All metrics are defined in `src/treeweft/infrastructure/metrics.py`. The exact
names below are the contract consumed by `assets/grafana/treeweft-freshness.json`
and `assets/prometheus/freshness-alerts.yml` — **do not rename without updating
both files**.

| Metric | Type | Labels | Meaning |
|---|---|---|---|
| `treeweft_source_index_stale` | Gauge 0/1 | `source_id` | 1 if indexed SHA ≠ current HEAD |
| `treeweft_source_index_age_seconds` | Gauge | `source_id` | Seconds since `source.indexed_at` |
| `treeweft_incremental_jobs_total` | Counter | `status` (done\|failed\|dead_letter) | Incremental job completions by outcome |
| `treeweft_incremental_job_duration_seconds` | Histogram | — | Runner wall-clock time (webhook pop → done/failed) |
| `treeweft_merge_to_searchable_seconds` | Histogram | — | Webhook-accept → incremental job done |
| `treeweft_queue_saturation` | Gauge 0..1 | — | `min(depth / MAX_QUEUE_DEPTH, 1.0)` |
| `treeweft_webhook_shed_total` | Counter | — | Webhooks rejected with 429 |
| `treeweft_dead_letter_jobs_total` | Counter | — | Jobs entering dead_letter state |
...
- **`FRESHNESS_SAMPLE_INTERVAL_SECONDS`** (default `60`) — how often the task
  ...
  `treeweft_source_index_stale` and `treeweft_source_index_age_seconds` per
  source.
...
- **Cardinality guard**: only label by `source_id` up to `FRESHNESS_MAX_SOURCES`
  ...
  aggregate `treeweft_sources_stale_total` gauge is exported instead.
...
Instead, Treeweft uses:

- `treeweft_source_index_stale` (stale flag: indexed SHA ≠ current HEAD)
- `treeweft_source_index_age_seconds` (age of the last index, i.e. how long
...
The Postgres `job_queue` depth is exposed as `treeweft_queue_saturation` in
[0, 1]:
...
2. Increments `treeweft_webhook_shed_total`.
...
in `jobs.payload` would waste memory and JSONB storage. Instead, Treeweft
...
**Symptom**: `treeweft_source_index_stale{source_id="…"} == 1` for > 1 hour
after a code push.
...
5. Is the queue saturated? Check `treeweft_queue_saturation` or `GET /status`.
...
**Symptom**: `treeweft_queue_saturation > 0.8` sustained; webhooks returning
429; `treeweft_webhook_shed_total` rising.

**Checklist**:
1. How many jobs are queued? `GET /status` shows queue depth.
2. Are jobs completing? Watch `rate(treeweft_incremental_jobs_total{status="done"}[5m])`.
3. Are jobs failing fast (error loop)? Watch `treeweft_incremental_jobs_total{status="failed"}`.
...
**Symptom**: `increase(treeweft_dead_letter_jobs_total[15m]) > 0`; alert
...
`treeweft_merge_to_searchable_seconds` above 300 s.
...
|---|---|---|
| `MAX_QUEUE_DEPTH` | 500 | Webhook 429 threshold; also denominator of `treeweft_queue_saturation` |
...
- **Grafana dashboard**: `assets/grafana/treeweft-freshness.json` — per-source
...
| Requirement | Notes |
|---|---|
| Running Treeweft indexer | Any backend (Milvus/LanceDB, GPU or CPU) |
| At least N indexed sources | Harness picks the first `--repos` from `GET /sources` |
| `httpx` (stdlib-only otherwise) | Already a core treeweft dep — `pip install -e .` |
| Bearer token | Only when `AUTH_ENABLED=true`; pass `--token` or set `$TREEWEFT_KEY` |

No GPU, no Milvus required — the simple profile (`TREEWEFT_PROFILE=simple`)
works fine.
...
|---|---|---|
| `--url` | `http://localhost:8001` | Indexer base URL (env: `TREEWEFT_INDEXER_URL`) |
| `--token` | `$TREEWEFT_KEY` | Bearer token for auth |
...
|---|---|---|
| `treeweft_merge_to_searchable_seconds` | freshness histogram (§3) | Canonical latency (logged as mean) |
| `treeweft_queue_saturation` | backpressure gauge (§4) | Peak saturation reported in summary |
| `treeweft_webhook_shed_total` | backpressure counter (§4) | 429 shed count delta |
...
```
────────────────────────────────────────────────────────────
  TREEWEFT INCREMENTAL LOAD TEST — SUMMARY
────────────────────────────────────────────────────────────
  Run ID  : c3d4e5f6-...
  URL     : http://localhost:8001
...
DATABASE_URL=postgresql://.../treeweft_loadtest MILVUS_COLLECTION=treeweft_loadtest \
  uvicorn treeweft.indexer_service:app --port 8002
...
- **`TREEWEFT_GIT_ALLOW_LOOPBACK=true`** — the indexer refuses git remotes on
...
```bash
DATABASE_URL=postgresql://.../treeweft_loadtest \
MILVUS_COLLECTION=treeweft_loadtest VECTOR_DIM=768 \
EMBEDDING_URLS=http://localhost:8082 \  # reachable backend, NOT the .env overflow URLs
GRAPH_STORE=sqlite GRAPH_DB_PATH=/tmp/treeweft_loadtest/graph.db \
WEBHOOK_GITHUB_SECRET=loadtest-secret WEBHOOK_TRUST_INLINE_FILES=1 AUTH_ENABLED=false \
TREEWEFT_GIT_ALLOW_LOOPBACK=true \
  uvicorn treeweft.indexer_service:app --port 8002
...
`treeweft_merge_to_searchable_seconds` histogram recorded all 200 as terminal.
