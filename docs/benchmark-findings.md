# Agentic Benchmark Findings: treeweft MCP search vs grep/read
...
"does treeweft help an LLM coding agent compared with grep?" Each section is dated and
...
3. **D1: symbol-free queries** — the query style, not the repo, was the confound;
   treeweft won on quality and cost when the identifier is absent from the question.
...
tools, `treeweft` = treeweft `search_code`); a **cell** is one run of every arm over one
...
grep / treeweft, `r@5` recall@5, `corr` correctness, `tok` mean tokens per query, and
...
**Question.** For an LLM agent answering questions about a codebase, does Treeweft's
...
but **grep/read is more token-efficient in most conditions** (treeweft ~1.4–2.7×). Treeweft's
retrieval **scales better with repo size** (it leads recall@10 on the large repo) and its
token premium is largely an engineering problem (weak recall@1 + bulky snippets). Treeweft's
...
- **treeweft arm:** one `search_code` tool → indexer `POST /search` (semantic + graph + rerank).
...
**Quality, two ways:** retrieval `recall@k`/`MRR` (files the agent read / treeweft returned vs
...
(chunks-only payload), `--context-window N` (prune transcript), `--no-scope-treeweft`
...
| cell | correctness g/t | recall@5 g/t | recall@10 g/t | tokens g/t | treeweft/grep |
|---|---|---|---|---|---|
...
(*fastapi symbol-containing was only measured **unscoped**: grep 10.9k/3.55 vs treeweft 28k/3.25,
2.58× — same grep-dominant pattern.*)

### Key sub-findings

1. **Token cost is dominated by `tool_observation` (75–85% for treeweft).** The agent re-reads
...
2. **Lean payloads backfired.** Dropping neighbors/community summaries and capping snippets
   (`--lean-search`) made treeweft *worse* (+53% tokens, lower quality): smaller per-call
...
3. **Context-window pruning helped** (`--context-window 6`): ~36% fewer treeweft tokens with
...
4. **Unscoped search was the biggest confound.** With no source filter, `search_code` hit the
   ...
   query). Scoping to `--repo` cut treeweft's tokens **−58%** and **tripled recall** on
...
5. **Retrieval scales with repo size.** Grep's recall@5 fell 0.85 → 0.50 from fastapi → godot
   (symbol-free); treeweft's held and **led recall@10 on godot (0.65 vs 0.50)**.

6. **The token premium is a recall@1 problem.** On symbol-containing godot, quality and
   recall@10 are tied, but treeweft's **recall@1 is 0.45 vs grep's 0.90** — the right file is
...
- **Answer quality: treeweft ≈ grep.** Tied on symbol-containing (both repos), modestly behind
  only on symbol-free. Treeweft rarely gives *worse* answers at equal scope.
- **Token cost: grep wins (treeweft 1.4–2.7× in 3 of 4 cells).** The one cheaper cell (fastapi
  symbol-free) was also lower quality.
- **Retrieval at scale: trends toward treeweft.** It loses recall on mid repos but ties
...
**Where treeweft is expected to win (not stressed here):** repos **not on disk** (grep can't
...
2. **Investigate recall@1** — treeweft's reranker isn't ranking the target file first on
...
- Both arms search/read on disk; treeweft additionally requires the index. The "not on disk"
...
python -m treeweft.benchmark queries --strategy cross_cutting,problem_driven \
...
python -m treeweft.benchmark queries --llm-symbol-free \
...
python -m treeweft.benchmark gold    --queries benchmarks/queries/<set>.jsonl --regen
python -m treeweft.benchmark agentic --queries benchmarks/queries/<set>.jsonl \
...
(the suffixes record the `--search-top-k 5` / `--context-window 6` / `--no-scope-treeweft`
...
**Why.** Every earlier number rested on **featbit** (the repo Treeweft was developed
...
### Results — grep vs treeweft (default reranker), n=100/repo

| Repo | grep | treeweft (default) | corr p (tl vs grep) |
|------|------|--------------------|---------------------|
...
Re-ran the treeweft arm against a search-only indexer with `RERANKER_PROVIDER=cohere`
(same Milvus/Neo4j, cached gold).

| Repo | treeweft (default) | treeweft (**Cohere**) |
|------|--------------------|------------------------|
...
Pooled treeweft-Cohere **vs grep**, 3 external repos (n=300, per-query sign test):
...
repos. There, treeweft is **quality-tied** with a plain grep/read agent, **behind on
...
Net for adoption: on the home repo Treeweft genuinely helps; on fresh repos under faithful
...
grep on retrieval — though featbit's queries are symbol-bearing too and treeweft still won
on quality there, so that is not the whole story. High tie counts (≈190/300 correctness)
mean both arms answer most clean queries equally. Treeweft's structural wins (repos not on
...
under-tests treeweft's semantic retrieval. The follow-up investigation traced the "doesn't
generalize" result to this: on the subset where grep *can't* match the symbol, treeweft
...
ground truth; validated grep-resistant). n=100/repo, grep vs treeweft (default reranker),
deepseek-chat.
...
|-----------------------------------|----------------|-------------|
| correctness | p=0.079 (featbit-driven) | **treeweft p<0.0001** |
| recall@5 | **grep** ahead, p=0.003 | treeweft ahead, p=0.056 |
| tokens | treeweft **+20%** | treeweft **−7.5% (cheaper)** |

### Conclusion
On the realistic "I don't know the symbol name" query — the case treeweft exists for —
**treeweft significantly beats a grep/read agent on answer quality (pooled p<0.0001) at
better token efficiency (−7.5%)**, and edges ahead on retrieval. PowerToys is the clearest:
grep collapses (recall 0.27, correctness 2.27, 43k tokens of failed greps) while treeweft
holds (0.53 / 3.39 / 32k). The earlier "parity-at-a-premium" result was an artifact of
symbol-bearing queries; **treeweft's value is semantic search, not symbol lookup** — grep
...
symbol-free) but still trends treeweft on correctness. The Cohere arm on the symbol-free
...
correctness — **within noise.** The index-quality gap was not suppressing treeweft.

**D3 — the value is the payload, not the rescoring.** Symbol-free, Cohere, treeweft:
...
does not replicate. Across deepseek-flash, claude-sonnet-5 and gpt-5.5, treeweft matched a
...
found treeweft significantly better on correctness on featbit — symbol-free p = 2.3e-10,
...
| agent | questions | cost vs grep | per-query cost W/L (p) | correctness grep → treeweft (p) | grep / treeweft 12-turn caps |
|---|---|---|---|---|---|
...
- **Turn-cap exhaustion.** On symbol-free questions treeweft caps out significantly less
  than grep on every model (p = 0.039, 0.0078, 0.00015).
- **vs claude-context** (deepseek-flash, same embedding model): treeweft ranks the right
