...
This guide is for operators running Treeweft across many repositories (tens to
...
- The `python -m treeweft.rebuild_graphs` CLI rebuilds the code graph for many sources in bulk.
...
The main [README](https://github.com/Treeweft/treeweft/blob/main/README.md) covers the rest of the indexer API (`CLAUDE.md` at
...
| `branch`       | git branch                                                     |
| `skip_patterns`| glob patterns to skip (honored only when `TREEWEFT_FEATURE_SKIP_PATTERNS=1`) |
...
python -m treeweft.fleet onboard repos.yaml --dry-run

# Index everything, then wait for all jobs to reach a terminal state
python -m treeweft.fleet onboard repos.yaml --wait

# Re-index the whole manifest
python -m treeweft.fleet onboard repos.yaml --force --wait
```
...
`TREEWEFT_KEY` (falls back to `TREEWEFT_MCP_API_KEY`). A token passed literally
...
`treeweft_fleet_refresh_enqueued_total` counter.
...
- **`?staleness=true` is capped** at `FLEET_STALENESS_MAX_SOURCES` (default
  ...
  gauges (`treeweft_source_index_stale`, `treeweft_sources_stale_total`) for
...
```bash
python -m treeweft.fleet health                       # table
python -m treeweft.fleet health --staleness --entities
python -m treeweft.fleet health --json                # raw rollup
```
...
**Simple mode is not a scale tier.** `TREEWEFT_PROFILE=simple` (embedded SQLite
