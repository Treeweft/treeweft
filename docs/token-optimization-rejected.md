...
This is the negative-results ledger for shrinking treeweft's search response. Read it
...
one tool configuration for the agent (`grep` = grep/glob/read tools; `treeweft` and its
variants = treeweft `search_code` in a given `response_mode`); a **cell** is one run of
...
**Question.** treeweft's search response is ~79% of an agent's token cost (measured:
...
**Short answer.** **Don't trim the search payload. `treeweft-full` is already the
...
rises**, and the case for `treeweft-full` is *strongest* for the high-value frontier
models.
...
|---|---|---|---|---|---|
| **treeweft (full)** | **4.5** | **26,329** | **4.21** | 0.84 | **4** |
| treeweft-facet | 6.3 | 34,208 | 3.95 | 0.85 | 16 |
| treeweft-facet-hydrate | 7.8 | 41,798 | 3.72 | 0.87 | 29 |
...
3. **Use the sufficiency-proxy screen as a cheap pre-filter**
   (`src/treeweft/application/benchmark/screen.py`): one LLM call/query → answer-or-`NEED_MORE`, ~15 min
...
python -m treeweft.benchmark agentic \
  ...
  --arms grep,treeweft,treeweft-facet,treeweft-summary-tail \
