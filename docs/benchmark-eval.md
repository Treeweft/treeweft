# Benchmark eval: grep vs treeweft (with significance)

This document is for anyone who wants to measure, on their own repositories, whether
treeweft search helps an LLM coding agent compared with a plain grep/glob/read agent —
...
- **arm** — one tool configuration the agent runs with. `grep` gives the agent
  grep/glob/read_file tools; `treeweft` gives it treeweft's `search_code`.
...
Use this workflow when you want a reproducible, numbers-first answer to "does treeweft
...
1. **Repo indexed into treeweft.** Index with:
...
3. **Capable tool-calling LLM.** The agentic harness drives a real LLM agent through
   ...
   - `BENCHMARK_QUALITY=STANDARD` — selects the hosted `STANDARD` tier of
     `resolve_llm_config` (`src/treeweft/adapters/benchmark/llm_client.py`); the
...
4. **Repo on disk.** The grep arm needs the repo present at the path you pass to
   `--repo`. The treeweft arm calls the indexer `/search` endpoint. Both must be
...
5. **Pinned (non-floating) model IDs.** Agentic runs refuse floating model aliases
   ...
   `TREEWEFT_ALLOW_FLOATING_MODELS=1`; a permitted run stamps
...
```bash
python -m treeweft.benchmark queries \
  --strategy all \
...
identifier); on symbol-**free** queries treeweft significantly beats a grep/read agent on
answer quality (pooled correctness p<0.0001) at lower token cost. **treeweft is for
...
```bash
python -m treeweft.benchmark clean \
...
Run grep arm vs treeweft arm in parallel, with LLM-judge scoring:
...
- `--model` — the LLM for the measured agent arms (grep + treeweft). Prefer a
...
- `--lean-search` — strips neighbor and community-summary context from treeweft
...
- `--vector-only` — treeweft arm uses pure Milvus vector+rerank with no graph
...
```bash
python -m treeweft.benchmark rollup \
...
| tokens-saved % | `(grep_tokens - treeweft_tokens) / grep_tokens * 100` — positive = treeweft cheaper |
| token_ratio | `treeweft_tokens / grep_tokens` — below 1.0 = treeweft used fewer tokens |
...
could easily arise by chance even if treeweft is genuinely a few points better. Larger
...
| grep tok / treeweft tok | Per-arm mean total tokens |
...
invalidate: `2026-07-05T232746_featbit_summary.json` (clean-100: treeweft correctness
3.89 vs grep 3.41, sign test p=0.006, −44.5% tokens, treeweft read-recall@5 0.87 vs
0.70) and `2026-07-05T234145_featbit-symfree_summary.json` (symbol-free: treeweft 3.23
vs grep 2.13, p=2.3e-10, −87.7% tokens; grep hits the 12-turn cap on 25.8% of queries
vs treeweft's 0.8% — the strongest treeweft-vs-grep cell on record, consistent with
...
treeweft is 11–47% cheaper than grep in every cell with correctness statistically tied.
...
# 4. Run grep vs treeweft (generates and caches gold answers on first run)
python -m treeweft.benchmark agentic \
...
```
| Repo             |  n | grep tok | treeweft tok | saved% | r@5 win-rt | r@5 p | corr win-rt | corr p |
|------------------|---:|---------:|-------------:|-------:|-----------:|------:|------------:|-------:|
