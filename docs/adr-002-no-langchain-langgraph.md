...
> were on the date above; the root-level `src/treeweft/*.py` modules named here
> are now thin import shims over `src/treeweft/application/` and
> `src/treeweft/adapters/`. The decision stands: `pyproject.toml` declares no
> LangChain or LangGraph dependency.

## Context

Treeweft's stack contains several pieces that, on the surface, look like the
...
- An embedding client over TEI (HuggingFace Text Embeddings Inference)
  (`src/treeweft/embedder.py`, `src/treeweft/adapters/tei/embedding_proxy.py`).
...
`src/treeweft/adapters/` and orchestration stays as direct async calls in
`mcp_server.py` and `indexer_service.py` *[2026-09: now
`src/treeweft/application/retrieval.py`, called by the indexer's `/search`
...
- LangChain abstractions are leaky in the directions we care about
  ...
  `neo4j.<fn>`) use stable `treeweft.*` attribute names that the Grafana
...
- The benchmark harness (`python -m treeweft.benchmark`, code under
  `src/treeweft/application/benchmark/`) exercises the pipeline as a
...
- `src/treeweft/application/mcp_server.py` — the MCP tools (each a plain HTTP
  call to the indexer); the search pipeline itself is
  `src/treeweft/application/retrieval.py`.
- `src/treeweft/retriever.py` — Milvus hybrid search and reranker batching.
- `src/treeweft/adapters/llm_api/llm_adapter.py` — LLM call site and Postgres summary cache.
