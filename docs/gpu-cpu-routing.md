...
- `src/treeweft/adapters/tei/embedding_proxy.py` — **(current)** `EmbeddingProxy` does cost-aware routing (real HF tokenizer); GPU/CPU pin when max chunk tokens > `GPU_MAX_TOKENS`; reactive failover across backends. See `docs/adr-001-cost-aware-embedding-proxy.md`.
...
```python
from treeweft.indexer import CodeIndexer   # shim over adapters/tree_sitter/indexer.py
idx = CodeIndexer()
