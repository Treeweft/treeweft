...
This is a dated (2026-06-18) study for readers asking "is treeweft worth it in dollars,
on the model I actually use?" A **cell** is one benchmark run of both **arms** (the
agent with grep/glob/read tools vs. with treeweft `search_code`) over 100 queries on
one agent model; **correctness** is a 1–5 LLM-judge score against a cached gold answer.

**Question.** treeweft's whole reason to exist: does it reduce the **cost** and/or
...
**`treeweft-full` beats the grep/read agent on both cost (−6% to −24%) and quality
...
featbit-clean-100, `--obs-char-limit 16000`, arms `grep` vs `treeweft` (full),
...
| agent model | grep $/q | treeweft $/q | **cost saved** | corr (grep→tl) | turns (grep→tl) | recall@5 | quality/USD |
|---|---|---|---|---|---|---|---|
...
others are directional (treeweft wins more queries than it loses) but dominated by
...
- **treeweft is cheaper on every model**, by −6% (deepseek/sonnet) up to −24%
  (gpt-5.5). Note treeweft's *payload* is similar-or-larger than grep's, yet it's
...
- **treeweft is at least as good on quality, usually better** (+0.2 to +0.3 mean
  correctness, higher recall@5), and never worse.
- **quality-per-dollar favours treeweft on every model** (e.g. gpt-5.5 68 vs 49,
...
therefore carry a caching-efficiency caveat; the treeweft-vs-grep *delta* does not**
...
| model | cost for a full 100-query cell (grep + treeweft) |
|---|---|
| deepseek-chat | **~$0.39** |
| gpt-5.5 | ~$15 |
...
python -m treeweft.benchmark agentic \
  ...
  --arms grep,treeweft --model <MODEL> --judge-model deepseek-chat \
