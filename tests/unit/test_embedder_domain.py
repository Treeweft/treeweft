...
`treeweft.embedder.embed` delegates to the module-level EmbeddingProxy;
`set_proxy()` is the documented test seam (no HTTP).
"""
import pytest

from treeweft.adapters.tei import embedding_proxy
...
async def test_embed_single_text(stub_proxy):
    from treeweft.embedder import embed
