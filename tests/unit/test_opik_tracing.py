...
def ot(monkeypatch):
    ...
    sys.modules.pop("treeweft.adapters.benchmark.opik_tracing", None)
    return importlib.import_module("treeweft.adapters.benchmark.opik_tracing")
...
def test_enabled_when_opik_track_set(ot, monkeypatch):
    ...
    sys.modules.pop("treeweft.adapters.benchmark.opik_tracing", None)
    mod = importlib.import_module("treeweft.adapters.benchmark.opik_tracing")
...
def test_tags_include_arm_repo_and_search_url(ot):
    tags = ot._tags("treeweft", "featbit", "run1",
                    {"search_url": "http://localhost:8002"})
    assert "arm:treeweft" in tags
