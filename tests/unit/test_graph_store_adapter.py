...
async def test_store_graph_calls_neo4j():
    ...
    from treeweft.graph_store import store_graph
    ...
    with patch("treeweft.adapters.neo4j.graph_store._get_session", return_value=mock_session):
...
async def test_clear_all_calls_neo4j(mock_neo4j_driver):
    ...
    from treeweft.graph_store import clear_all
    await clear_all()
...
async def test_get_entities_by_file(mock_neo4j_driver):
    ...
    from treeweft.graph_store import get_entities_by_file
...
async def test_traverse_returns_list(mock_neo4j_driver):
    ...
    from treeweft.graph_store import traverse
