...
import pytest
from treeweft.application import indexer_runners as _runners
from treeweft.application import lifecycle as _life
from treeweft.application import indexer_state as _tlstate
...
class TestDeadLetterEndpoints:
    ...
    def test_list_dead_letter_returns_dead_letter_jobs(self, mock_milvus_collection, mock_neo4j_driver):
        ...
        with patch("treeweft.application.indexer_state._job_store") as mock_store:
            mock_store.list_dead_letter = AsyncMock(return_value=[dl_job])

            from treeweft.indexer_service import app
    ...
    def test_retry_endpoint_requeues_dead_letter_job(self, mock_milvus_collection, mock_neo4j_driver):
        ...
        with patch("treeweft.application.indexer_state._job_store") as mock_store, \
             patch("treeweft.application.indexer_state._job_queue") as mock_queue, \
             patch("treeweft.application.indexer_runners._persist_job",
...
class TestComputeSourcStaleness:
    ...
    async def test_stale_when_shas_differ(self):
        from treeweft.application.indexer_runners import compute_source_staleness
        ...
        with patch(
            "treeweft.application.indexer_runners._git_head_sha",
            return_value="def456",
        ):
    ...
    async def test_uses_ls_remote_for_url_backed_source(self):
        ...
        with patch(
            "treeweft.application.indexer_runners._git_remote_sha",
            return_value="bbb",
        ) as mock_remote:
            result = await compute_source_staleness(src)
...
class TestFreshnessSampler:
    ...
    async def test_sampler_sets_stale_gauge_for_stale_source(self):
        ...
        with patch(
            "treeweft.application.indexer_service.graph_store.list_sources",
            new_callable=AsyncMock,
            return_value=sources,
        ), patch(
            "treeweft.application.indexer_runners.compute_source_staleness",
            side_effect=fake_staleness,
        ), patch(
            "treeweft.application.lifecycle._FRESHNESS_MAX_SOURCES",
...
class TestFreshnessLabelPruning:
    ...
    async def test_vanished_source_label_is_removed_next_tick(self):
        ...
        with patch(
            ...
            "treeweft.application.lifecycle._FRESHNESS_MAX_SOURCES", 1000,
        ), patch(
            "treeweft.application.indexer_state._job_queue", None,
        ), patch.object(
    ...
    async def test_aggregate_mode_prunes_all_per_source_labels(self):
        ...
        with patch(
            ...
            "treeweft.application.lifecycle._FRESHNESS_MAX_SOURCES", 2,
        ), patch.object(
...
class TestSamplerQueueSaturation:
    """FIX 2: the freshness sampler keeps treeweft_queue_saturation fresh."""
    ...
    async def test_sampler_sets_queue_saturation_from_depth(self):
        ...
        with patch(
            ...
            "treeweft.application.indexer_state._job_queue", fake_queue,
        ), patch(
            "treeweft.application.indexer_state.MAX_QUEUE_DEPTH", 500,
        ):
            await _life._sample_freshness_once()
...
class TestIncrementalMetrics:
    ...
    def test_records_done_counter_and_duration(self):
        from treeweft.application.indexer_runners import _record_incremental_metrics
        from treeweft.infrastructure import metrics
...
class TestBackpressure:
    ...
    def test_webhook_returns_429_when_queue_saturated(self, client, monkeypatch):
        ...
        with patch("treeweft.adapters.webhook.github.GitHubAdapter.handle_webhook",
                   return_value=mock_payload), \
             patch("treeweft.application.indexer_service.graph_store.list_sources",
                   new_callable=AsyncMock, return_value=[existing_source]), \
             patch("treeweft.application.indexer_state._job_queue") as mock_queue, \
             patch("treeweft.application.indexer_state.MAX_QUEUE_DEPTH", 1):
    ...
    def test_webhook_accepts_when_below_max_queue_depth(self, client, monkeypatch):
        ...
        with patch("treeweft.adapters.webhook.github.GitHubAdapter.handle_webhook",
             ...
             patch("treeweft.application.indexer_runners._persist_job",
                   new_callable=AsyncMock), \
             patch("treeweft.application.indexer_state.MAX_QUEUE_DEPTH", 500):
    ...
    def test_oversized_changed_files_falls_back_to_full_reindex(self, client, monkeypatch):
        ...
        with patch("treeweft.adapters.webhook.github.GitHubAdapter.handle_webhook",
             ...
             patch("treeweft.application.indexer_service.graph_store.list_sources",
                   new_callable=AsyncMock, return_value=[existing_source]), \
             patch("treeweft.application.indexer_state._job_queue") as mock_queue, \
             patch("treeweft.application.routes_webhook._find_active_job_for_source",
                   new_callable=AsyncMock, return_value=None), \
             patch("treeweft.application.indexer_runners._persist_job",
                   new_callable=AsyncMock), \
             patch("treeweft.application.indexer_state.MAX_QUEUE_DEPTH", 500), \
             patch("treeweft.application.routes_webhook.MAX_CHANGED_FILES", 5):
    ...
    def test_normal_incremental_job_enqueued_not_create_task(self, client, monkeypatch):
        ...
        with patch("treeweft.adapters.webhook.github.GitHubAdapter.handle_webhook",
             ...
             patch("treeweft.application.indexer_runners._persist_job",
                   new_callable=AsyncMock), \
             patch("treeweft.application.indexer_state.MAX_QUEUE_DEPTH", 500), \
             patch("treeweft.application.routes_webhook.MAX_CHANGED_FILES", 5000):
            mock_queue.depth = AsyncMock(return_value=0)
...
class TestQueueDepth:
    ...
    async def test_depth_returns_count_from_db(self):
        ...
        with patch(
            "treeweft.adapters.queue.postgres_queue.get_pool",
            new_callable=AsyncMock,
