...
def _reset_module():
    ...
    import treeweft.infrastructure.tracing as tracing_mod
...
def test_init_tracer_is_idempotent(monkeypatch):
    ...
    from treeweft.infrastructure import tracing
...
def test_get_tracer_returns_a_context_manager(monkeypatch):
    ...
    tracing.init_tracer("test-service")
    tracer = tracing.get_tracer("treeweft.test")
    ...
    with tracer.start_as_current_span("smoke") as span:
        span.set_attribute("treeweft.test", "ok")
        span.add_event("hi")
