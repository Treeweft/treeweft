...
`.env.example` ships ``TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS=`` (blank means
"use the derived default"). The dotenv loader in ``treeweft/__init__`` exports
...
def test_blank_retention_falls_back_to_derived_default(monkeypatch, raw):
    monkeypatch.setenv("TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS", raw)
...
def test_unset_retention_falls_back_to_derived_default(monkeypatch):
    monkeypatch.delenv("TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS", raising=False)
    assert lifecycle._login_purge_retention() == _DERIVED_DEFAULT


def test_explicit_retention_is_honoured(monkeypatch):
    monkeypatch.setenv("TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS", "86400")
...
def test_zero_retention_still_means_default(monkeypatch):
    ...
    monkeypatch.setenv("TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS", "0")
    assert lifecycle._login_purge_retention() == _DERIVED_DEFAULT
