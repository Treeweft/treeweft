...
class TestNoModuleImportsStateByName:
    ...
    def test_no_source_file_imports_a_state_name(self):
        root = pathlib.Path("src/treeweft")
        offenders = []
    ...
    def test_state_module_has_no_application_imports(self):
        ...
        tree = ast.parse(
            pathlib.Path("src/treeweft/application/indexer_state.py").read_text()
        )
        bad = [
            n.module for n in ast.walk(tree)
            if isinstance(n, ast.ImportFrom)
            and n.module
            and n.module.startswith("treeweft.application")
        ]
...
class TestServiceKeepsNoLocalCopies:
    def test_indexer_service_defines_no_state_names(self):
        ...
        tree = ast.parse(
            pathlib.Path("src/treeweft/application/indexer_service.py").read_text()
        )
        defined = set()
...
class TestSplitModulesStayAcyclic:
    ...
    def test_leaf_does_not_import_the_service(self, leaf):
        tree = ast.parse(
            (pathlib.Path("src/treeweft/application") / leaf).read_text()
        )
        bad = []
    ...
    def test_the_service_does_import_them(self):
        ...
        src = pathlib.Path("src/treeweft/application/indexer_service.py").read_text()
        assert "from treeweft.application import indexer_state" in src
        assert "from treeweft.application import indexer_runners as runners" in src
        assert "from treeweft.application import indexer_authz as authz" in src
...
class TestSplitModulesImportEachOtherByModuleOnly:
    ...
    def test_no_name_imported_from_a_sibling(self, module):
        path = pathlib.Path("src/treeweft/application") / f"{module}.py"
    ...
    def test_every_split_module_is_importable(self):
        ...
        for m in self.SPLIT:
            importlib.import_module(f"treeweft.application.{m}")
