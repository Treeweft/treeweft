...
async def test_generate_hyde_calls_llm(mock_llm_httpx):
    ...
    from treeweft.llm import generate_hyde
...
async def test_generate_summary_calls_llm(mock_llm_httpx):
    ...
    from treeweft.llm import generate_summary
...
async def test_llm_unreachable_graceful_fallback():
    ...
    with patch("treeweft.adapters.llm_api.llm_adapter._get_client", return_value=error_client):
