...
class TestFleetRefreshTickExecutes:
    ...
    def test_full_tick_enqueues_a_stale_source(self):
        ...
        with patch.object(lifecycle._state, "_job_queue", object()), \
             ...
             patch("treeweft.application.indexer_service._all_source_dicts",
                   new=AsyncMock(return_value=[src])), \
             patch("treeweft.application.indexer_runners.compute_source_staleness",
                   new=AsyncMock(return_value={"is_stale": True})), \
             patch("treeweft.application.routes_webhook._find_active_job_for_source",
                   new=AsyncMock(return_value=None)), \
             patch("treeweft.application.indexer_service._enqueue_source_reindex",
