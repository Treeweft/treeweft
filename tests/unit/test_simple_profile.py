...
def _clean_env(tmp_path) -> dict:
    ...
    env = {
        ...
        "HOME": str(tmp_path),  # ~/.treeweft lands in the tmp dir
        "TREEWEFT_PROFILE": "simple",
...
_PROFILE_PROBE = textwrap.dedent(
    ...
    import treeweft
    treeweft  # the import above already ran _apply_profile_defaults()
...
_IMPORT_PROBE = textwrap.dedent(
    """
    import treeweft.application.indexer_service  # the whole startup surface
    import treeweft.retriever
    import treeweft.graph_store
    import treeweft.embedder
    import treeweft.reranker
...
def test_startup_warns_when_auth_disabled(monkeypatch, caplog):
    ...
    from treeweft.application.lifecycle import _warn_if_auth_disabled
