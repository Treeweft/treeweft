...
import pytest
from treeweft.application import routes_auth as _rauth
from treeweft.application import indexer_authz as _authz
from fastapi.testclient import TestClient

from treeweft.domain.authorization import (
...
def _patch_hmac(mocker):
    ...
    mocker.patch(
        "treeweft.adapters.authorization.user_store.hash_api_key",
        side_effect=_mock_hash,
    )
    mocker.patch(
        "treeweft.application.auth_middleware.hash_api_key",
        side_effect=_mock_hash,
    )
    mocker.patch(
        "treeweft.application.indexer_service.hash_api_key",
        side_effect=_mock_hash,
        create=True,
    )
...
def _patch_password(mocker, verify_result: bool = True):
    mocker.patch(
        "treeweft.adapters.authorization.user_store.verify_password",
        return_value=verify_result,
    )
    mocker.patch(
        "treeweft.adapters.authorization.user_store.hash_password",
        return_value="$2b$12$fakehash",
    )
    mocker.patch(
        "treeweft.application.indexer_service.hash_password",
        return_value="$2b$12$fakehash",
        create=True,
    )
    mocker.patch(
        "treeweft.application.indexer_service.verify_password",
        return_value=verify_result,
        create=True,
    )
...
def _setup_admin_auth(mocker, admin_user: User):
    ...
    from treeweft.application.indexer_state import _user_store
...
def _setup_session_store(mocker, session_result=None):
    from treeweft.application.indexer_state import _session_store
...
def _setup_token_store(mocker):
    from treeweft.application.indexer_state import _token_store
...
def _setup_api_key_store(mocker):
    from treeweft.application.indexer_state import _api_key_store
...
class TestAuthLogin:
    ...
    async def test_login_success_sets_cookie(self, mocker):
        ...
        from treeweft.application.indexer_service import (
            app,
        )
        from treeweft.application.indexer_state import (
            _user_store,
            _session_store,
        ...
        assert "treeweft_session" in resp.cookies
    ...
    async def test_login_rate_limited_returns_429(self, mocker):
        ...
        mocker.patch.dict(
            os.environ,
            {"AUTH_ENABLED": "true", "TREEWEFT_LOGIN_MAX_ATTEMPTS": "5"},
        )

        from treeweft.application import indexer_service as svc
        from treeweft.application.indexer_service import app
        from treeweft.application.indexer_state import _login_attempt_store
...
class TestAuthLogout:
    ...
    async def test_logout_clears_cookie(self, mocker):
        ...
        from treeweft.application.indexer_service import app
        from treeweft.application.indexer_state import _user_store, _session_store
        ...
        resp = client.post(
            "/auth/logout",
            cookies={"treeweft_session": "some-token"},
        )
...
class TestPATCreate:
    ...
    async def test_pat_create_rejects_over_privileged_scopes(self, mocker):
        ...
        from treeweft.application.indexer_service import app
        from treeweft.application.indexer_state import _user_store, _token_store
...
class TestRequireScope:
    ...
    async def test_scope_gate_blocks_search_only_token_from_index(self, mocker):
        ...
        """
        mocker.patch.dict(os.environ, {"AUTH_ENABLED": "true"})
        from treeweft.application.indexer_authz import _require_scope
...
class TestSessionTtlParsing:
    ...
    def test_valid_value(self, mocker):
        from treeweft.application.routes_auth import _session_ttl_hours
        mocker.patch.dict(os.environ, {"TREEWEFT_SESSION_TTL_HOURS": "24"})
        assert _session_ttl_hours() == 24

    def test_malformed_value_falls_back(self, mocker):
        from treeweft.application.routes_auth import _session_ttl_hours
        mocker.patch.dict(os.environ, {"TREEWEFT_SESSION_TTL_HOURS": "12h"})
        assert _session_ttl_hours() == 12

    def test_nonpositive_falls_back(self, mocker):
        from treeweft.application.routes_auth import _session_ttl_hours
        mocker.patch.dict(os.environ, {"TREEWEFT_SESSION_TTL_HOURS": "0"})
        assert _session_ttl_hours() == 12
...
class TestAdminBootstrapCoexistence:
    ...
    async def test_key_and_password_seed_single_admin_with_both(self, mocker):
        ...
        mocker.patch.dict(os.environ, {
            "TREEWEFT_ADMIN_KEY": "k123",
            "TREEWEFT_BOOTSTRAP_ADMIN_USER": "root",
            "TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD": "pw",
        })
        from treeweft.application.lifecycle import (
    ...
    async def test_skips_when_users_exist(self, mocker):
        mocker.patch.dict(os.environ, {"TREEWEFT_ADMIN_KEY": "k"})
        from treeweft.application.lifecycle import (
            _seed_admin_if_first_start,
        )
        from treeweft.application.indexer_state import (
