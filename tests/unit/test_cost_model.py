...
from treeweft.domain.benchmark.cost_model import (
    compute_cost, cost_breakdown, price_for, PRICES,
)
from treeweft.domain.benchmark.agent_metrics import TokenAccount, aggregate
...
def test_aggregate_reports_cost_and_ratio():
    rows = [{"arms": {"grep": _arm(2000, 400, 4.0),
                      "treeweft": _arm(1500, 300, 4.5)}} for _ in range(3)]
    s = aggregate(rows, ["grep", "treeweft"], repo="x", model="deepseek-chat")
    assert s["grep"]["mean_cost_usd"] > 0
    assert s["treeweft"]["mean_cost_usd"] > 0
    assert s["grep"]["cost_verified"] is True
    assert s["grep"]["quality_per_usd"] > 0
    comp = s["comparison"]
    assert comp["cost_ratio_treeweft_over_grep"] is not None
    # treeweft cheaper here -> positive savings
