"""TREEWEFT_HMAC_SECRET must be set when auth is on.
...
from treeweft.infrastructure.config import ConfigError, required_settings, validate_config
...
class TestWhyItMatters:
    ...
    def test_two_processes_disagree_without_the_secret(self):
        a = _hash_in_subprocess({"TREEWEFT_HMAC_SECRET": ""})
        b = _hash_in_subprocess({"TREEWEFT_HMAC_SECRET": ""})
        assert a != b, (
            "expected per-process random secrets to produce different hashes"
        )

    def test_two_processes_agree_with_it(self):
        env = {"TREEWEFT_HMAC_SECRET": "a-shared-secret-value"}
...
class TestItIsRequiredWhenAuthIsOn:
    def test_listed_as_required_when_auth_enabled(self, monkeypatch):
        ...
        assert "TREEWEFT_HMAC_SECRET" in required_settings()
    ...
    def test_not_required_when_auth_is_off(self, monkeypatch, value):
        ...
        assert "TREEWEFT_HMAC_SECRET" not in required_settings()
    ...
    def test_startup_fails_loudly_when_auth_on_and_secret_missing(self, monkeypatch):
        ...
        monkeypatch.setenv("AUTH_ENABLED", "true")
        monkeypatch.delenv("TREEWEFT_HMAC_SECRET", raising=False)
        with pytest.raises(ConfigError, match="TREEWEFT_HMAC_SECRET"):
            validate_config()
    ...
    def test_empty_string_does_not_satisfy_it(self, monkeypatch):
        ...
        for var in required_settings():
            monkeypatch.setenv(var, "set")
        monkeypatch.setenv("TREEWEFT_HMAC_SECRET", "")
        with pytest.raises(ConfigError, match="TREEWEFT_HMAC_SECRET"):
            validate_config()
