...
from treeweft.domain.authorization import User, Role, UserStorePort
...
class TestSeedAdminOnFirstStart:
    ...
    async def test_seed_admin_when_no_users(self, mocker):
        """When count_users returns 0 and TREEWEFT_ADMIN_KEY is set,
    ...
    async def test_no_seed_when_users_exist(self, mocker):
        ...
        mocker.patch.dict(os.environ, {"TREEWEFT_ADMIN_KEY": "some-key"})
    ...
    async def test_no_seed_when_no_env_key(self, mocker):
        """When count_users is 0 but TREEWEFT_ADMIN_KEY is not set, no seed."""
        _patch_hmac(mocker)
        from treeweft.application.indexer_service import app
        from treeweft.application.indexer_state import _user_store
