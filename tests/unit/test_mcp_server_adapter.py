...
async def test_search_code_via_proxy(mocker, mock_search_response):
    ...
    from treeweft.application.mcp_server import search_code
...
async def test_search_code_enhanced_via_proxy(mocker, mock_search_response):
    ...
    from treeweft.application.mcp_server import search_code_enhanced
    _mock_async_client_post(mocker, json_data=mock_search_response)
...
async def test_explain_code_via_proxy(mocker, mock_search_response):
    ...
    from treeweft.application.mcp_server import explain_code
    _mock_async_client_post(mocker, json_data=mock_search_response)
...
async def test_graph_explore_via_proxy(mocker, mock_search_response):
    ...
    from treeweft.application.mcp_server import graph_explore
    _mock_async_client_post(mocker, json_data=mock_search_response)
...
async def test_find_definition_via_proxy(mocker, mock_entity_list):
    ...
    from treeweft.application.mcp_server import find_definition
...
async def test_find_callers_via_proxy(mocker, mock_entity_list):
    ...
    from treeweft.application.mcp_server import find_callers
...
async def test_find_references_via_proxy(mocker):
    ...
    from treeweft.application.mcp_server import find_references
...
async def test_mcp_index_tool_does_not_lend_service_key_by_default(mocker, monkeypatch):
    ...
    monkeypatch.setenv("TREEWEFT_MCP_API_KEY", "test-api-key-123")
    monkeypatch.delenv("TREEWEFT_MCP_SERVICE_ACCOUNT", raising=False)
    from treeweft.application.mcp_server import index_repo
...
async def test_mcp_index_tool_uses_service_key_when_opted_in(mocker, monkeypatch):
    ...
    but only behind an explicit TREEWEFT_MCP_SERVICE_ACCOUNT=true opt-in."""
    monkeypatch.setenv("TREEWEFT_MCP_API_KEY", "test-api-key-123")
    monkeypatch.setenv("TREEWEFT_MCP_SERVICE_ACCOUNT", "true")
    from treeweft.application.mcp_server import index_repo
    _mock_async_client_post(
        mocker, json_data={"job_id": "j1", "status": "queued", "source_id": "s1"}
    )
...
async def test_mcp_read_tool_does_not_leak_service_key(
    ...
    monkeypatch.setenv("TREEWEFT_MCP_API_KEY", "service-secret")
    from treeweft.application.mcp_server import search_code
    _mock_async_client_post(mocker, json_data=mock_search_response)
...
async def test_mcp_auth_no_key_no_header(mocker, mock_search_response, monkeypatch):
    """No Authorization header sent when TREEWEFT_MCP_API_KEY is not set."""
    monkeypatch.delenv("TREEWEFT_MCP_API_KEY", raising=False)
    from treeweft.application.mcp_server import search_code
    _mock_async_client_post(mocker, json_data=mock_search_response)
...
def test_sse_endpoint_exists():
    ...
    from treeweft.mcp_server import mcp
    assert mcp.name == "treeweft"
...
async def test_index_graph_proxies(mocker):
    ...
    from treeweft.application.mcp_server import index_graph
    mock_post = _mock_async_client_post(
...
async def test_rebuild_all_graphs_enqueues_every_source(mocker):
    ...
    from treeweft.application.mcp_server import rebuild_all_graphs
    sources = [
...
def test_parse_search_response_sources_and_provenance_fields():
    ...
    from treeweft.application.mcp_server import _parse_search_response
...
def test_search_response_to_dict_round_trips_provenance():
    ...
    from treeweft.application.mcp_server import (
