"""Shared pytest fixtures for treeweft unit tests.

Mock only out-of-process dependencies (TEI, Milvus, Neo4j, LLM API).
Never mock internal treeweft classes — Principle VIII (Detroit-style).
"""
...
def _fake_neo4j_entity(
    *,
    ...
    """Build a Neo4j entity dict matching the treeweft Entity schema."""
    return {
        "id": id,
        "type": type,
        "name": name,
...
def _make_milvus_search_hit(
    *,
    ...
    """Build a single Milvus search-hit dict matching the treeweft schema."""
    return {
        "id": 1,
        "distance": score,
...
def mock_milvus_collection(mocker) -> MagicMock:
    ...
    # Patch the module-level _client used by treeweft.adapters.milvus.vector_store.
    mocker.patch.object(
        __import__("treeweft.adapters.milvus.vector_store", fromlist=["_get_client"]),
        "_get_client",
        return_value=mock_mc,
    )
    # Reset cached default adapter so it picks up the patched _get_client
    vs = __import__("treeweft.adapters.milvus.vector_store", fromlist=["_get_adapter"])
...
def mock_neo4j_driver(mocker) -> AsyncMock:
    ...
    Uses the _fake_neo4j_entity helper so results match the treeweft schema.
    """
    ...
    mocker.patch.object(
        __import__("treeweft.adapters.neo4j.graph_store", fromlist=["_driver"]),
        "_driver",
        mock_driver,
    )
    return mock_driver
...
def mock_llm_httpx(mocker) -> AsyncMock:
    ...
    # Patch the module-level _client in treeweft.adapters.llm_api.llm_adapter.
    mocker.patch.object(
        __import__("treeweft.adapters.llm_api.llm_adapter", fromlist=["_get_client"]),
        "_get_client",
