"""CLI entry: python -m treeweft.backfill_provenance [options].
...
Examples:
  ...
  python -m treeweft.backfill_provenance --dry-run

  # Apply the backfill
  python -m treeweft.backfill_provenance

  # Re-resolve even rows that already have a commit_sha
  python -m treeweft.backfill_provenance --force
"""
...
from treeweft.application.indexer_runners import _git_head_sha

logger = logging.getLogger("treeweft.backfill_provenance")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from treeweft.adapters.postgresql.connection import close_pool, init_pool
    from treeweft.adapters.sources.repository import PostgreSourceRepository
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m treeweft.backfill_provenance",
