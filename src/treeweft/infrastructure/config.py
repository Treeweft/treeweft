...
def required_settings() -> list[str]:
    ...
    time so profile defaults applied in treeweft/__init__ are honored.
    """
...
def load_yaml_config(path: str | Path | None = None) -> dict[str, Any]:
    ...
    Searches for ``treeweft.yaml`` relative to the package directory by default,
    or uses the explicitly provided *path*.

    Args:
        path: Optional explicit path to a YAML config file.  If not given,
              ``src/treeweft/treeweft.yaml`` relative to this file's location
              is used.
    ...
    """
    if path is None:
        # Default path: src/treeweft/treeweft.yaml (next to this config module)
        default = Path(__file__).resolve().parent.parent / "treeweft.yaml"
        path = default
    else:
        path = Path(path)
...
FEATURE_SKIP_PATTERNS = os.environ.get("TREEWEFT_FEATURE_SKIP_PATTERNS", "") == "1"
