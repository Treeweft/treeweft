"""OpenTelemetry tracing for the Treeweft indexer.
...
The default tracer name used everywhere is `treeweft.indexer`; callers
that want a sub-namespace should ask for one explicitly via
`trace.get_tracer("treeweft.<adapter>")`.
"""
...
def init_tracer(service_name: str = "treeweft-indexer", app=None) -> None:
    ...
    version = os.environ.get("TREEWEFT_VERSION", "")
    if version:
...
def get_tracer(name: str = "treeweft.indexer"):
