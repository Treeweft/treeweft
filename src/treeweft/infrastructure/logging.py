"""Structured logging for Treeweft — JSON-formatted fatal error logs."""
...
def configure_logging(json_file: str | None = None) -> None:
    ...
    if not any(getattr(h, "_treeweft_handler", False) for h in root.handlers):
        handler = logging.StreamHandler(out)
        handler._treeweft_handler = True
        handler.setFormatter(logging.Formatter(
            "%(asctime)s %(levelname)s %(name)s %(message)s"
        ))
        root.addHandler(handler)
    # Root stays at WARNING so third-party INFO (httpx per-request lines,
    # neo4j notifications) doesn't flood; treeweft's own hierarchy logs INFO.
    if root.level in (logging.NOTSET, 0) or root.level > logging.WARNING:
        root.setLevel(logging.WARNING)
    logging.getLogger("treeweft").setLevel(logging.INFO)


def get_logger(name: str | None = None) -> structlog.stdlib.BoundLogger:
    return structlog.get_logger(name or "treeweft")


# Pre-configured logger for the indexer
log = get_logger("treeweft.indexer")
