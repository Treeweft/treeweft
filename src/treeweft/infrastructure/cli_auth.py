...
Precedence: --key-file, then --key, then $TREEWEFT_KEY, then
$TREEWEFT_MCP_API_KEY. Environment first would silently ignore an explicit
flag.
"""
...
def add_key_arguments(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: $TREEWEFT_KEY or $TREEWEFT_MCP_API_KEY)"
        ),
    )
    parser.add_argument(
        "--key-file",
...
def resolve_key(key: str | None, key_file: str | None = None) -> str | None:
    ...
    return (
        os.environ.get("TREEWEFT_KEY")
        or os.environ.get("TREEWEFT_MCP_API_KEY")
        or None
    )
