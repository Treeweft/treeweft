"""CLI entry: python -m treeweft.fleet <subcommand> [options].
...
bearer token. Provide it via --key or the TREEWEFT_KEY env var (falling back
to TREEWEFT_MCP_API_KEY).
...
Examples:
  ...
  python -m treeweft.fleet health --json
"""
...
from treeweft.domain.sources.manifest import parse_manifest
...
def _resolve_key(arg_key: str | None, key_file: str | None = None) -> str | None:
    ...
    from treeweft.infrastructure.cli_auth import resolve_key
...
def _add_common(p: argparse.ArgumentParser) -> None:
    ...
    from treeweft.infrastructure.cli_auth import add_key_arguments
