...
list[tuple[index, raw_score]]` — so it drops into `treeweft.reranker`'s
...
async def _rerank_request(query: str, documents: list[str]) -> list[tuple[int, float]]:
    ...
    with get_tracer("treeweft.reranker").start_as_current_span(
        "rerank.cloud",
        attributes={
            "treeweft.reranker_provider": RERANKER_PROVIDER,
            "treeweft.reranker_model": model,
            "treeweft.doc_count": len(documents),
        },
    ):
