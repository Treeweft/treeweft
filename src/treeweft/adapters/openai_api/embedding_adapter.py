...
TEI proxy so `treeweft.embedder` can dispatch between them; the Postgres
...
async def _embed_batch(batch: list[str]) -> list[list[float]]:
    ...
    with get_tracer("treeweft.embedding").start_as_current_span(
        "openai.embed",
        attributes={
            "treeweft.embedding_model": EMBEDDING_MODEL,
