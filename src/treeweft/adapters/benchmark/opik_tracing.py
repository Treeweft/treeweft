...
def _client():
    ...
    if _CLIENT is None:
        ...
        os.environ.setdefault("OPIK_PROJECT_NAME", "treeweft-benchmark")
        _CLIENT = opik.Opik()
    return _CLIENT
...
def _log_run_inner(run_name, repo_name, queries, gold, rows, arms, config) -> None:
    client = _client()
    project = os.environ.get("OPIK_PROJECT_NAME", "treeweft-benchmark")
...
def _log_experiments(client, repo_name, queries, gold, rows, arms, config,
    ...
    ds_name = f"treeweft-bench:{repo_name}"
