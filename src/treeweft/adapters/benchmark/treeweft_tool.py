"""Benchmark adapter: the treeweft-arm `search_code` tool.
...
as the treeweft arm's 'retrieved files', in returned order, for recall/MRR) —
...
def search_code_tool(
    ...
    async def run(query: str = "", top_k: int = 10, **_):
        ...
        # Rejected payload trims (treeweft-trimmed arm): per-request flags
        ...
        try:
            ...
            return {"error": f"treeweft search failed: {e}"}
        ...
        # FastMCP init.
        from treeweft.application.mcp_server import _search_response_to_markdown
...
def hydrate_chunks_tool(
    ...
    async def run(ids=None, **_):
        ...
        try:
            ...
            return {"error": f"treeweft hydrate failed: {e}"}
        for fp in _file_paths(data):
            ap = os.path.realpath(fp)
            if ap not in search_log:
                search_log.append(ap)
...
def treeweft_arm_tools(
    ...
    if repo_path:
        from treeweft.adapters.benchmark.grep_tools import read_file_tool, glob_tool
        ...
        # entirely and the arm just reproduces treeweft-facet). glob stays for
        # file discovery.
        if not with_hydrate:
