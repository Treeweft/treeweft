...
order) into `search_log` for recall/MRR, mirroring the treeweft arm.
...
from treeweft.adapters.benchmark.mcp_stdio import MCPStdioClient
...
def claude_context_search_tool(
    ...
    async def run(query: str = "", top_k: int = 10, **_):
        ...
        # Same sweep semantics as the treeweft arm: a fixed top_k overrides
...
def claude_context_arm_tools(
    ...
    """search_code (claude-context) + the same read/glob helpers the treeweft
    arm gets, so the arms differ only in their search tool."""
    from treeweft.adapters.benchmark.grep_tools import glob_tool, read_file_tool
