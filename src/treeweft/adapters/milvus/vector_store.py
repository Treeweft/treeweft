...
class MilvusAdapter(VectorStorePort):
    ...
    async def _execute_with_reconnect(self, method, *args, max_retries: int = 3, **kwargs):
        ...
        _tracer = get_tracer("treeweft.milvus")
...
# treeweft.application.retrieval (store-agnostic). Existing imports of these
# names from this module keep working.
# ---------------------------------------------------------------------------
from treeweft.application.retrieval import (  # noqa: E402,F401
    GRAPH_ALPHA,
    GRAPH_BETA,
    GRAPH_DELTA,
    GRAPH_EPSILON,
