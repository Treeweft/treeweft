...
def _arm_means(rows: list[dict], arm: str, model: str | None = None) -> dict:
    ...
    from treeweft.domain.benchmark.cost_model import compute_cost, price_for
...
def aggregate(rows: list[dict], arms: list[str], *, repo: str, model: str) -> dict:
    ...
    # Legacy comparison block — kept verbatim for the grep vs treeweft pairing
    # (existing dashboards/scripts read these keys).
    if "grep" in arms and "treeweft" in arms:
        g = summary["grep"]["mean_total_tokens"] or 0.0
        t = summary["treeweft"]["mean_total_tokens"] or 0.0
        _corr_w, _corr_l, _corr_t = win_loss_tie(rows, "treeweft", "grep", "correctness")
        _rec5_w, _rec5_l, _rec5_t = win_loss_tie(rows, "treeweft", "grep", "recall@5")
        _corr_total = _corr_w + _corr_l + _corr_t
        _rec5_total = _rec5_w + _rec5_l + _rec5_t
        gc = summary["grep"].get("mean_cost_usd")
        tc = summary["treeweft"].get("mean_cost_usd")
        summary["comparison"] = {
            "token_ratio_treeweft_over_grep": round(t / g, 4) if g else 0.0,
            "tokens_saved_pct": round((g - t) / g * 100, 2) if g else 0.0,
            "cost_ratio_treeweft_over_grep": round(tc / gc, 4) if gc else None,
    ...
    # Generic pairwise comparisons for any arm combination (e.g. treeweft vs
