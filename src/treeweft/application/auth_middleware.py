...
Resolution order (first match wins):
  1. Session cookie ``treeweft_session`` (HttpOnly, HMAC-hashed)
  2. Bearer token:
...
# TREEWEFT_DEV_MODE escape hatch. IPv4 loopback is 127.0.0.0/8, but
...
def _synthetic_dev_admin() -> User:
    ...
    AUTH_ENABLED=false + TREEWEFT_DEV_MODE=1 + loopback client.
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    1. Tries session cookie (treeweft_session).
    ...
    async def dispatch(
        ...
        if os.environ.get("AUTH_ENABLED", "").lower() != "true":
            ...
            # TREEWEFT_DEV_MODE=1 are set AND the caller is on loopback,
            ...
            if (
                os.environ.get("TREEWEFT_DEV_MODE", "").lower() in ("1", "true")
