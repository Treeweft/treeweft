...
``TREEWEFT_CORS_ORIGINS`` (comma-separated exact origins) drives this:
...
def parse_cors_origins(raw: str | None) -> list[str]:
    """Parse ``TREEWEFT_CORS_ORIGINS`` into a list of exact origins.
...
def add_cors_middleware(app: FastAPI, raw_origins: str | None) -> list[str]:
    ...
    if "*" in origins:
        ...
        logging.getLogger(__name__).error(
            "TREEWEFT_CORS_ORIGINS contains '*'; ignoring the explicit list and "
