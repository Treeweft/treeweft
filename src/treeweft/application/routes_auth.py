...
from treeweft.adapters.postgresql.login_attempt_store import LoginThrottleContention
...
def _session_ttl_hours() -> int:
    ...
    raw = os.environ.get("TREEWEFT_SESSION_TTL_HOURS", "12")
    try:
        hours = int(raw)
    except (TypeError, ValueError):
        logger.warning(
            "Invalid TREEWEFT_SESSION_TTL_HOURS=%r — using default 12", raw
        )
        return 12
...
LOGIN_MAX_ATTEMPTS = int(os.environ.get("TREEWEFT_LOGIN_MAX_ATTEMPTS", "5"))


LOGIN_WINDOW_SECONDS = int(os.environ.get("TREEWEFT_LOGIN_WINDOW_SECONDS", "300"))
...
LOGIN_MAX_ATTEMPTS_PER_USERNAME = int(
    os.environ.get("TREEWEFT_LOGIN_MAX_ATTEMPTS_PER_USERNAME", "50")
)
...
async def auth_login(req: LoginRequest, request: Request, response: Response):
    ...
    TREEWEFT_LOGIN_MAX_ATTEMPTS failures within TREEWEFT_LOGIN_WINDOW_SECONDS,
    further attempts get a 429 + Retry-After before bcrypt even runs.
    """
    from treeweft.adapters.authorization.user_store import verify_password, hash_api_key
    from treeweft.domain.authorization import login_throttle
    import uuid as _uuid
...
async def auth_logout(request: Request, response: Response):
    ...
    cookie_val = request.cookies.get("treeweft_session")
    deleted = True
    if cookie_val:
        try:
    ...
    response.delete_cookie("treeweft_session")
    if not deleted:
        raise HTTPException(
            503,
...
async def auth_me(request: Request):
    ...
    if request.cookies.get("treeweft_session"):
        auth_method = "session"
