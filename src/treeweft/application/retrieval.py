...
store `treeweft.retriever` dispatches to (VECTOR_STORE env), reranks them,
...
from treeweft import community, embedder, graph_store, llm
from treeweft import reranker as _reranker
from treeweft.domain.shared import chunk_hit_from_milvus, make_hit_id
...
def _store():
    """The active vector-store module (env-dispatched in treeweft.retriever).

    Imported lazily per call: `treeweft.retriever` imports this module's
...
# queries keep the rich payload (that's where treeweft's value lives, D1).
...
def _apply_response_budget(
    ...
    from treeweft.domain.token_budget import TokenBudget, TokenBudgetConfig
...
async def hydrate_chunks(
    ...
    """
    from treeweft.domain.shared import parse_hit_id
