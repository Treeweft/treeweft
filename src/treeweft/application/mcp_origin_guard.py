...
Configuration — ``TREEWEFT_MCP_ALLOWED_ORIGINS`` (comma-separated exact
origins), mirroring ``TREEWEFT_CORS_ORIGINS`` on the indexer:
...
def parse_allowed_origins(raw: str | None) -> list[str]:
    """Parse ``TREEWEFT_MCP_ALLOWED_ORIGINS`` into a list of exact origins.
