...
async def _fleet_refresh_once() -> int:
    ...
    from treeweft.application import indexer_service as _svc
...
_LOGIN_PURGE_ENABLED = os.environ.get(
    "TREEWEFT_LOGIN_PURGE_ENABLED", "true"
).lower() not in ("0", "false", "no")


_LOGIN_PURGE_INTERVAL = float(
    os.environ.get("TREEWEFT_LOGIN_PURGE_INTERVAL_SECONDS", "3600")
)
...
def _login_purge_retention() -> int:
    ...
    (``.env.example`` ships ``TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS=``) and
    ``0`` all mean "derive it" — a blank used to reach ``int("")`` and abort
    the indexer at import.
    """
    raw = os.environ.get("TREEWEFT_LOGIN_PURGE_RETENTION_SECONDS", "").strip()
...
async def _run_freshness_sampler() -> None:
    """Periodically update treeweft_source_index_stale / _age_seconds gauges.

    Cardinality guard: if source count exceeds FRESHNESS_MAX_SOURCES, the
    per-source label is dropped and only the aggregate
    treeweft_sources_stale_total gauge is updated. This prevents label
...
def _prune_vanished_freshness_labels(current: set[str]) -> None:
    ...
    leave its ``treeweft_source_index_stale``/``_age_seconds`` series exported
...
async def _seed_admin_if_first_start() -> None:
    ...
    Checks both legacy TREEWEFT_ADMIN_KEY (API-key-based) and the new
    TREEWEFT_BOOTSTRAP_ADMIN_USER + TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD
    ...
    try:
        ...
        admin_key = os.environ.get("TREEWEFT_ADMIN_KEY", "")
        bootstrap_user = os.environ.get("TREEWEFT_BOOTSTRAP_ADMIN_USER", "")
        bootstrap_pass = os.environ.get("TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD", "")
        has_password = bool(bootstrap_user and bootstrap_pass)

        if not admin_key and not has_password:
            logger.warning(
                "No TREEWEFT_ADMIN_KEY or TREEWEFT_BOOTSTRAP_ADMIN_USER/"
                "TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD set — cannot seed admin user"
            )
            return

        from treeweft.adapters.authorization.user_store import (
        ...
        # TREEWEFT_ADMIN_KEY) and local username/password (from the bootstrap
        ...
        if user is not None and has_password:
            try:
                ...
                logger.error(
                    "TREEWEFT_BOOTSTRAP_ADMIN_PASSWORD exceeds bcrypt's "
        ...
        methods = []
        if api_key_hash:
            methods.append("TREEWEFT_ADMIN_KEY")
        if has_password:
...
async def startup(app):
    ...
    from treeweft.infrastructure.config import validate_config
    from treeweft.infrastructure.logging import configure_logging
    ...
    init_tracer("treeweft-indexer", app=app)
    validate_config()
    ...
    if _state.DATABASE_URL:
        from treeweft.adapters.postgresql.connection import init_pool
        from treeweft.adapters.postgresql import run_migrations
    ...
    if _state.DATABASE_URL and os.environ.get("EMBEDDING_PROVIDER", "tei") == "tei":
        ...
        from treeweft.adapters.tei.embedding_proxy import EmbeddingProxy, set_proxy
    ...
    if _LOGIN_PURGE_ENABLED:
        ...
        logger.info("Login-attempt purge disabled (TREEWEFT_LOGIN_PURGE_ENABLED=false)")
...
async def shutdown(app):
    ...
    try:
        from treeweft.adapters.postgresql.connection import close_pool
        await close_pool()
    except Exception:
        pass
