...
from treeweft.embedder import MAX_BATCH_SIZE, embed
from treeweft.graph_extractor import extract_graph
from treeweft.graph_store import delete_entities_by_file
from treeweft.indexer import CodeIndexer, detect_language, SUPPORTED_EXTENSIONS
...
async def _persist_job(job_dict: dict) -> None:
    ...
    if _state._job_store is not None:
        from treeweft.domain.jobs import Job
...
GIT_ALLOWED_HOSTS: frozenset[str] = frozenset(
    h.strip().lower()
    for h in os.environ.get("TREEWEFT_GIT_ALLOWED_HOSTS", "").split(",")
    if h.strip()
)
...
GIT_BLOCK_PRIVATE = os.environ.get("TREEWEFT_GIT_BLOCK_PRIVATE", "").lower() == "true"
...
GIT_FOLLOW_REDIRECTS = (
    os.environ.get("TREEWEFT_GIT_FOLLOW_REDIRECTS", "").lower() == "true"
)
...
GIT_ALLOW_LOOPBACK = (
    os.environ.get("TREEWEFT_GIT_ALLOW_LOOPBACK", "").lower() == "true"
)
...
def _resolve_inside_clone(repo_path: str, rel_path: str) -> str | None:
    ...
    (/tmp/treeweft_inc_A vs /tmp/treeweft_inc_AB) cannot pass.
    """
...
def _host_is_forbidden(host: str) -> bool:
    ...
    Loopback is refused unless TREEWEFT_GIT_ALLOW_LOOPBACK is set — see that
    constant for the one case that needs it.

    RFC1918 / unique-local is refused only under TREEWEFT_GIT_BLOCK_PRIVATE.
...
def _is_safe_git_url(url: str) -> bool:
    ...
    host on TREEWEFT_GIT_ALLOWED_HOSTS when one is configured.
    """
    if not url:
        return False
    if url.startswith("-"):
...
async def _graph_index_file(file_path: str, source_id: str):
    with tracer.start_as_current_span(
        "graph.index_file",
        attributes={"treeweft.file_path": file_path, "treeweft.source_id": source_id},
    ) as span:
        try:
        ...
        span.set_attribute("treeweft.entity_count", len(entities))
        span.set_attribute("treeweft.relationship_count", len(relationships))
        if entities:
    ...
    *,
    ...
    with tracer.start_as_current_span("parse_and_chunk") as span:
        try:
            ...
            span.set_attribute("treeweft.skipped", True)
            return 0  # UnicodeDecodeError — skip binary file gracefully
        span.set_attribute("treeweft.chunk_count", len(chunks) if chunks else 0)
    ...
    with tracer.start_as_current_span(
        "embed.chunks",
        attributes={
            "treeweft.chunk_count": len(chunks),
            "treeweft.max_chunk_bytes": max_bytes,
        },
    ):
    ...
    if USE_SUMMARY_VECTOR:
        ...
        if to_embed:
            with tracer.start_as_current_span(
                "embed.summaries",
                attributes={"treeweft.summary_count": len(to_embed)},
            ):
    ...
    with tracer.start_as_current_span(
        ...
        attributes={"treeweft.row_count": len(chunks)},
    ):
        ...
        async with sem:
            ...
            with tracer.start_as_current_span(
                ...
                attributes={
                    ...
                    "treeweft.file_size_bytes": file_size,
                    "treeweft.language": language,
                    "treeweft.source_id": source_id,
                    "treeweft.job_id": job.get("job_id", ""),
                },
            ) as file_span:
                try:
                ...
                file_span.set_attribute("treeweft.chunk_count", n)
...
async def _fail_job(job: dict, exc: BaseException):
    ...
    # Structured log
    from treeweft.infrastructure.logging import log as struct_log
    struct_log.error(
        "job_failed",
        job_id=job.get("job_id"),
...
async def _run_index_file_job(job: dict, file_path: str):
    ...
    job_attrs = {
        "treeweft.job_id": job.get("job_id", ""),
        "treeweft.kind": "file",
        "treeweft.source_id": job.get("source_id", ""),
        "treeweft.file_path": file_path,
        "treeweft.resuming": resuming,
    }
    with tracer.start_as_current_span("index_file", attributes=job_attrs) as span:
        try:
            ...
            span.set_attribute("treeweft.chunk_count", len(chunks))
            _log_progress(job, force=True)
            await _finalize_job(
                job, len(chunks), 1,
...
async def _run_index_directory_job(job: dict, directory: str, pattern: str):
    ...
    job_attrs = {
        "treeweft.job_id": job.get("job_id", ""),
        "treeweft.kind": "directory",
        "treeweft.source_id": job.get("source_id", ""),
        "treeweft.source_path": directory,
        "treeweft.resuming": resuming,
    }
    try:
        job["status"] = "running"
...
async def _run_index_repo_job(
    ...
    job_attrs = {
        ...
        "treeweft.kind": "repo",
        "treeweft.source_id": job.get("source_id", ""),
        "treeweft.source_path": req.path or "",
        "treeweft.source_url": req.url or "",
        "treeweft.source_branch": req.branch or "",
        "treeweft.resuming": resuming,
        "treeweft.force": bool(req.force),
    }
    try:
        job["status"] = "running"
        metrics.active_jobs.inc()
        ...
        if req.url:
            ...
            repo_path = tempfile.mkdtemp(prefix="treeweft_")
            ...
            with tracer.start_as_current_span(
                "git.clone",
                attributes={"treeweft.source_url": req.url, "treeweft.branch": branch or "", **job_attrs},
            ):
...
async def _run_incremental_index(
    ...
    try:
        ...
        if branch and branch.startswith("-"):
            raise ValueError(f"Unsafe git branch: {branch!r}")
        repo_path = tempfile.mkdtemp(prefix="treeweft_inc_")
        cleanup_path = repo_path
        kwargs: dict = {
            "depth": 1,
            "env": _git_env(),
