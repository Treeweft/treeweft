...
async def lifespan(app: FastAPI):
    init_tracer("treeweft-mcp", app=app)
    # TREEWEFT_MCP_SERVICE_ACCOUNT lets the service account stand in for a
    ...
    # unconditionally.
    if os.environ.get("TREEWEFT_MCP_SERVICE_ACCOUNT", "").lower() == "true":
        logging.getLogger(__name__).warning(
            "TREEWEFT_MCP_SERVICE_ACCOUNT=true on the HTTP transport: callers "
...
# caller's own bearer, or TREEWEFT_MCP_TOKEN, or — write/admin tools only,
# opt-in — the shared TREEWEFT_MCP_API_KEY service account); see docs/authz.md.
from treeweft.application.mcp_origin_guard import (
...
app.add_middleware(
    ...
    allowed_origins=parse_allowed_origins(
        os.environ.get("TREEWEFT_MCP_ALLOWED_ORIGINS")
    ),
)
