...
LOGIN_TRUST_FORWARDED_FOR = (
    os.environ.get("TREEWEFT_LOGIN_TRUST_FORWARDED_FOR", "0") == "1"
)
...
def _require_admin(request: Request):
    ...
    when `AUTH_ENABLED=false` AND `TREEWEFT_DEV_MODE=1` AND the request
...
_COOKIE_SECURE_MODE = os.environ.get("TREEWEFT_COOKIE_SECURE", "auto").strip().lower()
...
def _cookie_secure(request: Request) -> bool:
    ...
    TREEWEFT_LOGIN_TRUST_FORWARDED_FOR, the existing behind-a-trusted-proxy
...
def _dummy_password_hash() -> str:
    ...
    if _DUMMY_PASSWORD_HASH is None:
        from treeweft.adapters.authorization.user_store import hash_password
...
def _login_client_ip(request: Request) -> str:
    ...
    TREEWEFT_LOGIN_TRUST_FORWARDED_FOR=1 (we're behind a trusted reverse
...
async def _require_authenticated_user(request: Request):
    ...
    session_cookie = request.cookies.get("treeweft_session")
    if session_cookie and _state._session_store is not None:
        from treeweft.adapters.authorization.user_store import hash_api_key as _hak
        try:
    ...
    from treeweft.adapters.authorization.user_store import hash_api_key
    try:
...
def _search_auth_enforced() -> bool:
    ...
    off when TREEWEFT_SEARCH_OPEN is set — the escape hatch for a deployment
    that wants protected *indexing* (AUTH_ENABLED=true) but deliberately open
    *search* (the simple single-tier evaluator path).
    """
    if os.environ.get("AUTH_ENABLED", "").lower() != "true":
        return False
    if os.environ.get("TREEWEFT_SEARCH_OPEN", "").lower() in ("1", "true"):
        return False
    return True
...
def _write_auth_enforced() -> bool:
    ...
    Deliberately NOT `_search_auth_enforced()`: TREEWEFT_SEARCH_OPEN opens
...
async def _authorize_scope(
    ...
    endpoints pass `_write_auth_enforced()` so that TREEWEFT_SEARCH_OPEN — an
