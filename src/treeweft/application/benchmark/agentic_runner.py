"""Benchmark application: agentic runner — grep / treeweft / claude-context arms.
...
from treeweft.adapters.benchmark.claude_context_tool import (
    ClaudeContextClient,
    claude_context_arm_tools,
)
from treeweft.adapters.benchmark.code_graph_rag_tool import (
    CodeGraphRagClient,
    code_graph_rag_arm_tools,
)
from treeweft.adapters.benchmark.grep_tools import grep_arm_tools
from treeweft.adapters.benchmark.repomap_tool import build_repo_map, repomap_arm_tools
from treeweft.adapters.benchmark.treeweft_tool import treeweft_arm_tools
from treeweft.application.benchmark.agent_loop import AgentRunResult, run_agent
from treeweft.application.benchmark.gold import build_gold
...
from treeweft.domain.benchmark.metrics import canonicalize_retrieved
from treeweft.domain.benchmark.metrics import compute_mrr, recall_at_k
...
def enforce_floating_model_policy(
    ...
    Override with `--allow-floating-model` or TREEWEFT_ALLOW_FLOATING_MODELS=1;
    ...
    """
    from treeweft.adapters.benchmark.llm_client import is_floating_alias

    allow = allow or os.environ.get("TREEWEFT_ALLOW_FLOATING_MODELS", "") == "1"
    warnings: list[str] = []
    ...
    print("[agentic] refusing to start: benchmark runs must not use floating "
          ...
          "TREEWEFT_ALLOW_FLOATING_MODELS=1 to override.")
    raise SystemExit(2)
...
async def _run_arm(arm: str, query: str, repo: str, search_url: str,
    ...
    if arm == "grep":
    ...
    elif arm in ("treeweft", "treeweft-facet", "treeweft-summary-tail",
                 "treeweft-trimmed", "treeweft-st-brief",
                 "treeweft-facet-hydrate"):
        log = []
        # Opt-in response modes: the facet/summary-tail arms run
        # the treeweft arm with the respective server-side response_mode. The
        # facet arm relies on read_file (appended by treeweft_arm_tools when
        # repo_path is set) to fetch the body-less ranges it returns.
        # treeweft-facet-hydrate instead gets a batch hydrate_chunks tool
        # to fetch the dropped bodies by hit_id — the charitable facet variant.
        response_mode = {
            "treeweft": "full",
            "treeweft-facet": "facet",
            "treeweft-summary-tail": "summary_tail",
            "treeweft-trimmed": "full",
            "treeweft-st-brief": "summary_tail",
            "treeweft-facet-hydrate": "facet",
        }[arm]
        # Rejected payload trims (all three at once vs baseline `treeweft`):
        ...
        trims = None
        if arm == "treeweft-trimmed":
            trims = {
                "include_community_summaries": False,
                "adaptive_topk": True,
                "strip_imports": True,
            }
        elif arm == "treeweft-st-brief":
...
async def run_one_query(
    ...
    context_window: int | None = None, scope_treeweft: bool = True,
    ...
    runs = await asyncio.gather(
        *[_run_arm(a, q["query"], repo, search_url, agent_llm, max_turns,
                   search_top_k, lean_search, context_window, scope_treeweft,
...
async def run_agentic(
    ...
    scope_treeweft: bool = True, vector_only: bool = False,
    ...
    from treeweft.adapters.benchmark.llm_client import describe_config
    ...
    print("[agentic] treeweft search scoped to repo: "
          + (f"True (path_prefix={repo})" if scope_treeweft else "False (whole index)"))
    if vector_only:
    ...
    print(f"[agentic] treeweft mode: {mode}")
    for w in warnings:
        ...
        with open(rows_path, "a") as rows_file:
            for i, q in enumerate(queries, 1):
                ...
                row = await run_one_query(
                    ...
                    context_window=context_window, scope_treeweft=scope_treeweft,
    ...
    agg["treeweft_scoped"] = scope_treeweft
    agg["treeweft_vector_only"] = vector_only
    agg["treeweft_graph_scoring"] = graph_scoring and not vector_only
    if "repomap" in arms:
    ...
    from treeweft.adapters.benchmark import opik_tracing
    ...
    opik_tracing.log_run(
        ...
        config={
            ...
            "treeweft_scoped": scope_treeweft, "treeweft_vector_only": vector_only,
            "treeweft_graph_scoring": graph_scoring and not vector_only,
        },
    )
    return agg
