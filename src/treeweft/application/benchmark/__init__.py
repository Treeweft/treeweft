...
def _warn_curation(queries: list[dict], path: str) -> None:
    ...
    """
    from treeweft.domain.benchmark.curation import curation_summary
...
def cmd_queries(args):
    ...
    from treeweft.application.benchmark.query_gen import generate_queries
...
def _cmd_queries_enhanced(args):
    ...
    import difflib
    from treeweft.application.benchmark.enhanced_query_gen import (
        DEFAULT_EXCLUDES,
...
def _cmd_queries_llm_symfree(args):
    ...
    from treeweft.application.benchmark.enhanced_query_gen import DEFAULT_EXCLUDES
    from treeweft.application.benchmark.llm_query_gen import generate_symbol_free_queries
...
def cmd_run(args):
    ...
    from treeweft.application.benchmark.runner import run_benchmark
...
def cmd_ab(args):
    ...
    from treeweft.application.benchmark.ab_compare import ab_compare
...
def cmd_agentic(args):
    """Real agentic loop: grep / treeweft / claude-context arms (tokens + answer quality)."""
    from treeweft.domain.benchmark.queries import load_queries
    from treeweft.application.benchmark.agentic_runner import run_agentic
    from treeweft.domain.benchmark.rollup import format_comparison_table
    ...
    result = asyncio.run(run_agentic(
        ...
        scope_treeweft=not args.no_scope_treeweft, vector_only=args.vector_only,
...
def cmd_rollup(args):
    ...
    for path in args.summaries:
        with open(path) as f:
            summaries.append(upgrade_legacy_names(json.load(f)))
...
def cmd_clean(args):
    ...
    from treeweft.domain.benchmark.query_hygiene import (
        apply_quota,
        dedup,
        filter_answerable,
    )
...
def main():
    parser = argparse.ArgumentParser("treeweft.benchmark")
    ...
    p = sub.add_parser("agentic", help="Real agentic loop: grep vs treeweft vs claude-context arms")
    p.add_argument("--queries", required=True)
    p.add_argument("--repo", required=True, help="Repo root for grep/glob/read tools")
    p.add_argument("--search-url", default="http://localhost:8001")
    p.add_argument("--arms", default="grep,treeweft",
                   help="csv of arms to run: grep, treeweft, treeweft-facet, "
                        "treeweft-facet-hydrate, treeweft-summary-tail, claude-context, "
                        "repomap, code-graph-rag. The treeweft-facet arm runs treeweft with "
                        "response_mode=facet (metadata + chunk header, no code body; "
                        "the agent fetches ranges via read_file); treeweft-facet-hydrate "
                        " is the same facet payload but the agent gets a batch "
                        "hydrate_chunks tool to fetch dropped bodies by hit_id; "
                        "treeweft-summary-tail "
    ...
    p.add_argument("--model", default=None,
                   help="Model for the measured agent arms (grep + treeweft). "
    ...
    p.add_argument("--search-top-k", type=int, default=None,
                   help="Fix the treeweft search_code top_k (sweep payload size); "
                        "default = agent-chosen")
    p.add_argument("--lean-search", action="store_true",
                   help="Lean treeweft payload: chunks only (drop neighbors + "
    ...
    p.add_argument("--no-scope-treeweft", action="store_true",
                   help="Don't scope treeweft search_code to --repo (search the whole "
                        "multi-repo index). Default: scoped, matching the repo-scoped grep arm")
    p.add_argument("--vector-only", action="store_true",
                   help="treeweft arm = pure Milvus vector+rerank, no graph rescoring and "
    ...
    p.add_argument("--allow-floating-model", action="store_true",
                   help="Permit a floating model alias (deepseek-chat, bare "
                        ...
                        "into _summary.json. Env: TREEWEFT_ALLOW_FLOATING_MODELS=1.")
