...
tracer = get_tracer("treeweft.indexer")
...
from treeweft.indexer import (
    CodeIndexer,
    detect_language,
    SUPPORTED_EXTENSIONS,
)
from treeweft.retriever import init_collection, insert_chunks, delete_chunks_by_source
from treeweft.retriever import graph_search, hydrate_chunks
from treeweft.graph_extractor import extract_graph
...
from treeweft.graph_store import delete_entities_by_file
...
from treeweft.application.fleet import (
    build_fleet_health,
...
async def _lifespan(app: FastAPI):
    ...
    """
    from treeweft.application import lifecycle as _lc
...
# Public re-export: treeweft.indexer_service (the root shim) exposes `indexer`.
...
# CORS for the operator UI v2 SPA (cross-origin). Driven by TREEWEFT_CORS_ORIGINS;
# empty = permissive dev default. See treeweft.application.cors for why a wildcard
# origin must not be combined with credentials (it silently breaks cookie auth).
from treeweft.application.cors import add_cors_middleware

add_cors_middleware(app, os.environ.get("TREEWEFT_CORS_ORIGINS"))

# ── Auth middleware (US2) ──────────────────────────────────────────
# GET /sources and GET /jobs are public; POST/DELETE mutations require auth.
from treeweft.application.auth_middleware import AuthMiddleware
from treeweft.adapters.authorization.api_key_store import RevocationUnavailable
from treeweft.adapters.authorization.grant_store import GrantLookupUnavailable
...
from treeweft.adapters.authorization.search_audit_store import PostgreSQLSearchAuditStore
from treeweft.adapters.authorization.token_store import PostgreSQLPersonalAccessTokenStore
...
from treeweft.domain.authorization.access import AuthorizationService
from treeweft.domain.search_audit import SearchAuditRecord
...
class IndexDirectoryRequest(BaseModel):
    ...
    # Feature-flagged: when TREEWEFT_FEATURE_SKIP_PATTERNS=1 the indexer
...
from treeweft.adapters.postgresql.login_attempt_store import (
    LoginAttemptStore,
...
async def _run_preflight_for_path(path: str, *, concurrency: int | None = None) -> dict:
    ...
    from treeweft.preflight.scanner import walk_repo
    from treeweft.preflight.coefficients import load_coefficients, fallback_coefficients
    from treeweft.preflight.model import estimate_total
    from treeweft.preflight.recommender import build_report
...
async def preflight(req: PreflightRequest, request: Request):
    ...
    if req.url:
        ...
        target = tempfile.mkdtemp(prefix="treeweft_preflight_")
        cleanup_path = target
        kwargs: dict = {
            "depth": 1,
...
async def metrics_endpoint():
    from treeweft.infrastructure.metrics import get_metrics
...
async def list_job_groups(request: Request):
    ...
    """
    from treeweft.application.job_groups import summarize_group
...
from treeweft.domain.authorization import Role, scopes_for_role
...
async def remove_source(source_id: str, request: Request):
    ...
    INDEXER_HOST bind. It is NOT off under TREEWEFT_SEARCH_OPEN — that flag
...
MAX_QUERY_CHARS = int(os.environ.get("TREEWEFT_MAX_QUERY_CHARS", "8192"))
MAX_TOP_K = int(os.environ.get("TREEWEFT_MAX_TOP_K", "200"))
MAX_RERANK_POOL = int(os.environ.get("TREEWEFT_MAX_RERANK_POOL", "500"))
MAX_HYDRATE_IDS = int(os.environ.get("TREEWEFT_MAX_HYDRATE_IDS", "200"))
...
async def _attach_provenance(result: dict, *, check_staleness: bool) -> None:
    ...
    try:
        ...
        from treeweft.domain.provenance import build_provenance
...
if __name__ == "__main__":
    uvicorn.run("treeweft.indexer_service:app", host="0.0.0.0", port=8001)
