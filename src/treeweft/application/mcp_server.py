...
def _auth_headers() -> dict:
    """Service-account auth for INDEX/ADMIN tools — TREEWEFT_MCP_API_KEY."""
    key = os.environ.get("TREEWEFT_MCP_API_KEY", "")
    if key:
...
def _service_account_enabled() -> bool:
    ...
    bearer of its own.
    """
    return os.environ.get("TREEWEFT_MCP_SERVICE_ACCOUNT", "").lower() == "true"
...
def _env_token_headers() -> dict:
    ...
    credentials from the environment". Distinct from TREEWEFT_MCP_API_KEY,
    ...
    """
    token = os.environ.get("TREEWEFT_MCP_TOKEN", "").strip()
...
def _tool_auth_headers(ctx: "Context | None") -> dict:
    ...
    Precedence: caller bearer -> TREEWEFT_MCP_TOKEN -> gated service account
    -> nothing. The caller's own HTTP bearer wins when present (shared HTTP
    transport). Otherwise TREEWEFT_MCP_TOKEN — the caller's own credential
...
def _read_tool_auth_headers(ctx: "Context | None") -> dict:
    ...
    Precedence: caller bearer -> TREEWEFT_MCP_TOKEN -> nothing. Deliberately
    ...
    principal's own grants. `TREEWEFT_MCP_API_KEY` is a *shared* service
    account; if these reads could fall back to it, an operator who opts into
    the service account (`TREEWEFT_MCP_SERVICE_ACCOUNT=true`, intended for
    ...
    for writes, reopened on the read path. TREEWEFT_MCP_TOKEN is safe here
