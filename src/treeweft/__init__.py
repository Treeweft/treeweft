"""treeweft package — DDD architecture with backward-compatible legacy module access.

On import, best-effort load of a project-root `.env` so any process that
imports treeweft (uvicorn, REPL, scripts) sees the same vars docker-compose
...
def _alias_legacy_env(
    environ: MutableMapping[str, str], reported: set[str] | None = None
) -> set[str]:
    """Honor pre-rebrand TREELOOM_* vars as fallbacks for TREEWEFT_*.

    Each legacy var is copied to its new name unless the new name is already
    set to a non-empty value (the new name wins; empty counts as unset, since
    compose's `X=${X:-}` passes empty strings). Warns once per call about
    legacy names not already in `reported`, and returns the updated set so a
    second pass (after .env loads) only reports what the first pass missed.
    """
    reported = set() if reported is None else reported
    found = sorted(
        k for k in environ if k.startswith(_LEGACY_PREFIX) and k not in reported
    )
    for key in found:
        new_key = _PREFIX + key[len(_LEGACY_PREFIX):]
        if not environ.get(new_key):
...
def _load_dotenv_if_present() -> None:
    ...
    if os.environ.get("TREEWEFT_SKIP_DOTENV"):
        return
...
_alias_legacy_env(os.environ, _reported_legacy)


def _apply_profile_defaults() -> None:
    """Apply deployment-profile defaults (TREEWEFT_PROFILE=simple).
    ...
    """
    if os.environ.get("TREEWEFT_PROFILE", "") != "simple":
        return
    defaults = {
        # backend selection
        ...
        "DATABASE_URL": "postgresql://treeweft:treeweft_pass@localhost:5432/treeweft",
    }
...
# RERANKER_PROVIDER — so `from treeweft import X` and
# `from treeweft.X import fn` resolve to the same backend.
_LEGACY_MODULES = {
    "benchmark": "treeweft.application.benchmark",
    "community": "treeweft.adapters.graph.community_adapter",
    "config": "treeweft.infrastructure.config",
    "embedder": "treeweft.embedder",
    "graph_extractor": "treeweft.adapters.tree_sitter.graph_extractor",
    "graph_store": "treeweft.graph_store",
    "indexer": "treeweft.adapters.tree_sitter.indexer",
    "indexer_service": "treeweft.application.indexer_service",
    "llm": "treeweft.adapters.llm_api.llm_adapter",
    "main": "treeweft.application.main",
    "mcp_server": "treeweft.application.mcp_server",
    "models": "treeweft.domain.shared",
    "prompt_enhancer": "treeweft.domain.prompt_enhancer",
    "reranker": "treeweft.reranker",
    "retriever": "treeweft.retriever",
    "sources": "treeweft.domain.shared",
}
