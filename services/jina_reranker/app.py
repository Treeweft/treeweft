...
uses custom modeling code (no standard `model_type`), so the normal treeweft
...
a different reranker without changing any treeweft code.

Scores come from the model's own `compute_score` (sigmoid, [0,1]). That's fine
for treeweft: `rerank()` only sorts by score, and `graph_rescore` min-max
