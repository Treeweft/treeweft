...
`retriever.rerank` calls exactly this, so no treeweft code changes — just point
`RERANKER_URL` here.
...
docker logs --tail 20 treeweft-jina-reranker-1   # ends in "reranker ready"
```

Then point treeweft's reranker at it and restart the indexer:
...
- Scores are the model's `compute_score` (sigmoid, `[0,1]`). treeweft only sorts
