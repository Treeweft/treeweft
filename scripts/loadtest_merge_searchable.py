...
``TREEWEFT_GIT_ALLOW_LOOPBACK=true`` set. See docs/incremental-indexing.md §9.
...
Example::
    ...
    python scripts/loadtest_merge_searchable.py \\
        ...
        --base-path /tmp/treeweft_loadtest --repos 6 \\
...
def create_repo(base: Path, name: str) -> Path:
    ...
    (repo / "README.md").write_text(
        f"# {name} — throwaway treeweft merge->searchable fixture\n"
    )
    _git(repo, "init", "-q", "-b", "main")
    _git(repo, "config", "user.email", "loadtest@treeweft.local")
    ...
    try:
        ...
        for line in r.text.splitlines():
            ...
            if base == "process_resident_memory_bytes":
                out["rss_bytes"] = fval
            elif base == "treeweft_queue_saturation":
                out["queue_saturation"] = max(out.get("queue_saturation", 0.0), fval)
            elif base == "treeweft_webhook_shed_total":
                out["webhook_shed_total"] = out.get("webhook_shed_total", 0.0) + fval
            elif base == "treeweft_merge_to_searchable_seconds_sum":
                out["m2s_sum"] = fval
            elif base == "treeweft_merge_to_searchable_seconds_count":
                out["m2s_count"] = fval
    except Exception:  # noqa: BLE001
        pass
    return out
...
def _print_summary(s: dict, run_id: str, args: argparse.Namespace) -> None:
    ...
    line = "─" * 62
    print(f"\n{line}")
    print("  TREEWEFT MERGE→SEARCHABLE LOAD TEST — SUMMARY")
    print(line)
...
def main() -> int:
    ...
    p.add_argument("--base-path", default="/tmp/treeweft_loadtest",
