"""Load-test harness for Treeweft incremental (webhook) indexing.

Drives sustained synthetic ``POST /webhook`` traffic against a running Treeweft
...
treeweft dependency) and does NOT require GPU or Milvus.  It can target any
reachable indexer regardless of vector-store backend.

---------------------------------------------------------------------------
PREREQUISITES
---------------------------------------------------------------------------
1. A running Treeweft indexer on ``--url`` (default http://localhost:8001).
...
- ``treeweft_merge_to_searchable_seconds`` histogram — if published by the
...
- ``treeweft_queue_saturation`` gauge — peak value over the run.
- ``treeweft_webhook_shed_total`` counter — 429 responses.
...
# httpx is a core treeweft dependency — no extra install needed.
try:
    import httpx
except ImportError:  # pragma: no cover
    sys.exit(
        "httpx is required but not installed.\n"
        "It is a core treeweft dependency — run: pip install -e ."
    )
...
def run_loadtest(
    ...
    shed_counter_start = baseline_metrics.get("treeweft_webhook_shed_total", 0.0)
    ...
    shed_counter_end = final_metrics.get("treeweft_webhook_shed_total", 0.0)
    queue_sat_final = final_metrics.get("treeweft_queue_saturation", -1.0)

    # Also check treeweft_merge_to_searchable_seconds histogram if available
    # (prefer it as canonical latency when present)
    histogram_p95 = -1.0
    histogram_count_key = "treeweft_merge_to_searchable_seconds_count"
    if histogram_count_key in final_metrics and final_metrics[histogram_count_key] > 0:
        ...
        hist_sum = final_metrics.get("treeweft_merge_to_searchable_seconds_sum", 0)
        hist_count = final_metrics.get(histogram_count_key, 1)
        histogram_mean = hist_sum / hist_count if hist_count else 0
        print(
            f"\n  [metrics] treeweft_merge_to_searchable_seconds "
    ...
    for t_val in (
        baseline_metrics.get("treeweft_queue_saturation"),
        final_metrics.get("treeweft_queue_saturation"),
    ):
...
def _print_summary(res: LoadTestResult) -> None:
    ...
    sep = "─" * 60
    print(f"\n{sep}")
    print("  TREEWEFT INCREMENTAL LOAD TEST — SUMMARY")
    print(sep)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Load-test harness for Treeweft incremental indexing.\n"
    ...
    p.add_argument(
        "--url",
        default=os.environ.get("TREEWEFT_INDEXER_URL", "http://localhost:8001"),
        help="Indexer base URL (default: $TREEWEFT_INDEXER_URL or http://localhost:8001)",
    )
    p.add_argument(
        "--token",
        default=os.environ.get("TREEWEFT_KEY") or os.environ.get("TREEWEFT_MCP_API_KEY"),
        help="Bearer token for auth (default: $TREEWEFT_KEY / $TREEWEFT_MCP_API_KEY)",
    )
    p.add_argument(
        "--secret",
