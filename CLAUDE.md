...
- **Never import `adapters/milvus/vector_store` or `adapters/neo4j/graph_store` from startup-path code** — both `require_env` their service vars at import and crash simple mode. Go through the root shims (`treeweft/retriever.py`, `graph_store.py`, `embedder.py`, `reranker.py`); `tests/unit/test_simple_profile.py` guards this.
...
- **Milvus collection has an explicit schema** (`vector`, `summary_vector` zero-filled on insert when absent — **not** a real nullable column despite the field name, server-side BM25 `sparse_vector`). Schema-changing edits require dropping the collection and re-indexing every source, and must also bump `INDEX_SCHEMA_VERSION` and regenerate `contracts/index_schema.json`.
- **New routes under `/search`, `/hydrate-chunks`, `/find-*`, `/graph-*`, `/index-*` or `/build-community` must call `index_guard.require_searchable()`/`require_writable()`** (ADR-004 §3) immediately after authorization. `tests/unit/test_index_gate_routes.py`'s route-classification check fails on any unclassified route under those prefixes, so a missed gate is caught, not silently shipped.
- **Never delete `(:TreeweftMeta)` in Neo4j or the `treeweft_meta` row in SQLite.** That is the index schema stamp (ADR-004 §3) — `clear_all()`/`clear_index_data()` in both graph adapters already exclude it; any new bulk-delete Cypher/SQL must too.
...
- **Do NOT set `TREEWEFT_MCP_TRANSPORT=sse` in a shared `.env`** — `src/treeweft/__init__.py` loads the repo `.env` from any cwd, so it would turn the `treeweft-mcp` stdio script into a second, colliding SSE server. The HTTP+SSE container is toggled only by `COMPOSE_PROFILES=http-mcp`.
...
- **Never edit a registered prompt version in `adapters/llm_api/prompts.py`.** Add a new version and its `tests/unit/fixtures/prompt_hashes.json` entry; `test_prompt_registry.py` guards this. Making a new version a fresh install's default needs a benchmark run (constitution III).
